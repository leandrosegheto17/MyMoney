...
-- antes desta pipeline começar (implementação anterior do stakeholder, ver
-- ADR-012) — este arquivo NUNCA é reexecutado contra esse projeto (o CLI já
-- registra as 13 versões `*_baseline_legacy` como aplicadas em
-- `supabase_migrations.schema_migrations`, controle por número de versão, não por
-- conteúdo/checksum).
--
-- Até esta correção, este arquivo era um placeholder vazio ("Conteudo real nao
-- versionado neste repositorio") — gap real de disaster recovery: um ambiente
-- NOVO (`supabase db push`/`db reset` contra um projeto Supabase vazio, restauração
-- real de backup) não recriaria nenhuma tabela/função/policy/trigger do produto,
-- porque as 13 migrations `*_baseline_legacy` (a única cobertura cronológica
-- anterior a `BE-M-01`) não continham DDL nenhum.
--
-- Conteúdo real abaixo: dump schema-only do estado ATUAL do projeto real,
-- gerado via `supabase db dump --linked -f supabase/schema-baseline-legacy.sql`
-- (mesmo comando sugerido pelo DevSecOps/CTO, Bloqueio 011) — cobre tanto o
-- schema legado quanto tudo que as migrations reais deste repositório
-- (`BE-M-00` a `BE-M-13`) já adicionaram até 2026-09-03, consolidado em UM único
-- arquivo, na posição cronológica mais antiga (esta), para que um `db push`
-- contra um projeto novo recrie o schema inteiro antes de qualquer migration
-- subsequente (`20260902100000_be_m01...` em diante) rodar. Os outros 12
-- arquivos `*_baseline_legacy.sql` (que originalmente cobriam o mesmo intervalo
-- de "schema legado, sem DDL real conhecido") passam a ser no-ops explícitos —
-- ver comentário em cada um — em vez de repetir este dump 12 vezes.
--
-- `schema-baseline-legacy.sql` (raiz de `supabase/`) mantém o mesmo conteúdo como
-- referência legível/versionada e agora também é referenciado em
-- `supabase/config.toml` (`db.migrations.schema_paths`), para uso pelo workflow
-- de schema declarativo (`supabase db diff`) — não é o mecanismo de restore em
-- si (que é `db push` replaying migrations, coberto por ESTE arquivo), mas
-- fecha a segunda metade literal do achado do DevSecOps.
--
-- Nenhuma linha real de dado é tocada por este arquivo (é só DDL — tabelas
-- recriadas aqui nascem vazias; dado real só existe hoje no projeto já linkado,
-- onde este arquivo não roda de novo).
