// F1-BE-11 — Edge Function `webauthn-register` (SDD.md §3).
//
// Cerimônia de REGISTRO de uma credencial WebAuthn para desbloqueio local por
// dispositivo. Duas ações, ambas exigindo um JWT Supabase válido (o usuário já
// passou pelo login completo e-mail+senha antes de poder registrar biometria —
// ver SDD.md §3):
//
//   - action "generate-options": gera as opções para `navigator.credentials
//     .create()` no client (tarefa do Frontend, F1-FE-05). Não grava nada no
//     banco — só lê as credenciais já registradas do usuário (para preencher
//     `excludeCredentials` e evitar recadastro do mesmo autenticador).
//   - action "verify": recebe o `attestationResponse` retornado por
//     `navigator.credentials.create()`, valida com `verifyRegistrationResponse`
//     e, se válido, insere a linha em `webauthn_credentials` via service role.
//
// Nota de arquitetura — desafio (challenge) sem estado entre invocações:
//   WebAuthn exige que o `challenge` usado na cerimônia de "generate-options"
//   seja o mesmo apresentado de volta em "verify". Como uma Edge Function não
//   pode depender de estado em memória entre invocações (skill
//   `arquitetura-serverless`) e esta migration não criou uma tabela de
//   desafios pendentes, o challenge é *stateless*: um token auto-verificável
//   (payload JSON + assinatura HMAC-SHA256, chave derivada de
//   `SUPABASE_SERVICE_ROLE_KEY`) com validade curta (90s), amarrado ao
//   `user_id` autenticado e à finalidade ("registration"). "verify" recalcula
//   o HMAC e confere validade/usuário/finalidade — nenhum dado é persistido
//   nem mantido em memória entre a geração e a verificação.
//   Mitigação de replay aplicada (BE-M-09, `BLOCKERS.md` Bloqueio 006 —
//   "mitigar agora", veredito do CTO): o esquema HMAC acima prova a
//   integridade/validade do challenge (assinatura, `exp`, `uid`, `purpose`),
//   mas não bastava por si só — nada impedia a MESMA dupla challenge+
//   assertion capturada de ser reenviada repetidamente dentro da janela de
//   90s. `public.webauthn_challenges` (migration
//   `20260902100600_be_m09_webauthn_challenges.sql`) resolve isso como
//   registro de consumo único: "generate-options" insere uma linha
//   (`challenge`, `ceremony_type`, `expires_at`); "verify" extrai o
//   `challenge` do `clientDataJSON` da própria resposta e faz um
//   `UPDATE ... WHERE consumed_at IS NULL AND expires_at > now() RETURNING`
//   atômico, ANTES de chamar `verifyRegistrationResponse` — se nenhuma linha
//   for afetada (challenge já consumido, expirado, ou nunca emitido), a
//   verificação é rejeitada com 409 sem sequer tentar validar a assinatura
//   WebAuthn. Ver `consumeChallenge`/`persistChallenge` abaixo.
//
// Logging: JSON estruturado (timestamp, level, function_name, request_id,
// user_id) — nunca loga `public_key`/`attestationResponse` brutos, só ids e
...
const RP_NAME = Deno.env.get("WEBAUTHN_RP_NAME") ?? "MyMoney";
const ALLOWED_ORIGINS = (Deno.env.get("WEBAUTHN_ORIGIN") ?? "http://localhost:5173")
  .split(",")
  .map((origin) => origin.trim())
  .filter((origin) => origin.length > 0);

const FUNCTION_NAME = "webauthn-register";
...
Deno.serve(async (req) => {
  ...
  try {
    if (action === "generate-options") {
      const { data: existingCredentials, error: existingError } = await withTimeout(
      ...
      if (existingError) {
        log("error", { requestId, userId: user.id }, "Falha ao consultar credenciais existentes", {
          db_error_code: existingError.code ?? null,
        });
        return jsonResponse(
          { error: "internal_error", message: "Não foi possível preparar o registro. Tente novamente." },
          500,
          origin,
        );
      }

      const challenge = await createChallenge({ uid: user.id, purpose: "registration" });
      ...
      const options = await generateRegistrationOptions({
        ...
        userDisplayName: user.email ?? "Usuário MyMoney",
        challenge,
        attestationType: "none",
        excludeCredentials: (existingCredentials ?? []).map((row) => ({ id: row.credential_id as string })),
      ...
      log("info", { requestId, userId: user.id }, "Opções de registro WebAuthn geradas", {
        existing_credentials_count: existingCredentials?.length ?? 0,
      });
    ...
    if (action === "verify") {
      ...
      const deviceLabel = typeof body.deviceLabel === "string" ? body.deviceLabel.slice(0, 255) : null;

      if (!attestationResponse || typeof attestationResponse !== "object") {
        log("warn", { requestId, userId: user.id }, "attestationResponse ausente ou inválido no corpo");
        return jsonResponse(
          { error: "invalid_body", message: "attestationResponse é obrigatório." },
          400,
          origin,
        );
      }

      const challengeString = decodeClientDataChallenge(attestationResponse.response?.clientDataJSON);
      if (!challengeString) {
        log("warn", { requestId, userId: user.id }, "clientDataJSON sem challenge decodificável");
        return jsonResponse(
          { error: "invalid_body", message: "attestationResponse inválido." },
          400,
          origin,
        );
      }
      ...
      // chamar verifyRegistrationResponse — rejeita qualquer reenvio da mesma
      // dupla challenge+assertion dentro da janela de 90s (replay).
      ...
      try {
        ...
        return jsonResponse(
          { error: "internal_error", message: "Não foi possível concluir o registro. Tente novamente." },
          500,
          origin,
        );
      }
        ...
        return jsonResponse(
          {
            ...
            message: "Este desafio já foi utilizado, expirou, ou é inválido. Solicite um novo registro.",
          },
          409,
          origin,
        );
      ...
      try {
        verification = await verifyRegistrationResponse({
          ...
          expectedChallenge: (challenge) => isValidChallenge(challenge, { uid: user!.id, purpose: "registration" }),
          expectedOrigin: ALLOWED_ORIGINS,
          expectedRPID: RP_ID,
        });
      } catch (err) {
        log("warn", { requestId, userId: user.id }, "attestationResponse rejeitado na verificação", {
          error: err instanceof Error ? err.message : String(err),
        });
        return jsonResponse(
      ...
      if (!verification.verified) {
        log("warn", { requestId, userId: user.id }, "Registro WebAuthn não verificado");
        return jsonResponse(
          { error: "verification_failed", message: "Não foi possível validar a credencial informada." },
          400,
          origin,
      ...
      const { error: insertError } = await withTimeout(
        admin.from("webauthn_credentials").insert({
          user_id: user.id,
          credential_id: credential.id,
          public_key: toPostgresBytea(credential.publicKey),
      ...
      if (insertError) {
        // 23505 = unique_violation (credential_id já registrado) — trata como
        // reexecução segura em vez de erro genérico (skill arquitetura-serverless,
        // item de idempotência).
        if (insertError.code === "23505") {
          log("warn", { requestId, userId: user.id }, "Credencial já registrada anteriormente", {
            credential_id: credential.id,
          });
          return jsonResponse(
            { error: "already_registered", message: "Esta credencial já está registrada." },
            409,
            origin,
          );
        }

        log("error", { requestId, userId: user.id }, "Falha ao gravar credencial WebAuthn", {
      ...
      log("info", { requestId, userId: user.id }, "Credencial WebAuthn registrada com sucesso", {
        credential_id: credential.id,
      });

      return jsonResponse({ success: true, credentialId: credential.id }, 200, origin);
    }
