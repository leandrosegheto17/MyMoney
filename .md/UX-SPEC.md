...
**Nota (Lote 5, 2026-09-18) — `AuthCard`/`AuthLayout` passam de "recomendado" a
implementável**: sem mudar a especificação mínima acima, o Tech Lead registra a
superfície esperada (nome final segue decisão do `frontend`): `AuthCard` com slots
`eyebrow` (ex. "Passo 1 de 2", contraste ≥4,5:1 — nunca `--text-3`, Seção 5), título
`h1`, `description`, corpo e `footer`; `size` `sm|md`; `align` `left|center`.
`S-AUTH-03/05` (Unlock) deixa de ser o padrão borderless e passa a usar o mesmo
`AuthLayout`/`AuthCard`, encerrando os "2 padrões de container" do Achado 3 (Seção
3.0.1). Emoji "🔒" do Unlock substituído por ícone `lucide-react` `aria-hidden`.
Não há artboard de Auth; a referência visual é esta Seção 3.2 + tokens (3.1).
Nenhum componente novo além dos dois já listados.
