...
| FE-DEBT-07 | Estabilizar testes de UI que estouram o timeout de 5s sob carga da máquina (achado da validação de FE-DEBT-06): CaptureFab (confirmar rascunho), DraftReviewBanner (falha ao confirmar), GoalsPage (aporte), PaymentMethodsPage (cadastro), TransactionsPage (fluxo normal), SettingsPage (alterar PIN), AccountsPage/CreditCardsPage/ExportReportPage e PinSetupPage (divergente, reproduzido também em `main`). Causa provável: `testTimeout` padrão de 5s com `userEvent` + jsdom + axe em `setup.ts`; avaliar `testTimeout`/`hookTimeout` global e limite de workers em `vitest.config`, ou `waitFor`/`findBy` sincronizando o fim de cada fluxo, sem enfraquecer asserções | Frontend | `QA-REPORT.md` Seção 31 (S-1) | Suíte completa sem timeout em 3 execuções seguidas sob carga moderada (com outros processos Node ativos); nenhuma asserção removida ou relaxada | 0.25 dia (prazo: antes do fechamento do Lote 6) | Não iniciada | Refatoração Lote-5 |
