...
| BE-F3-02 | Edge Function de suporte a captura por voz: recebe transcrição do client (Web Speech API) e/ou aciona fallback STT em nuvem, extrai campos estruturados | Backend | RF-F3-01 AC1, ADR-006 | Campos extraídos retornam marcados como "sugestão automática, não confirmada" (AC1) | 1.5 dia | **Concluída — 2026-09-07.** `DIR-33` checado antes de escrever código: `supabase functions list` no projeto linkado não mostrou nenhuma function de voz pré-existente (só `auth-email-mfa`/`webauthn-*`/`backup-export`/`invoice-close`/`recurring-generate`/`fixed-bill-generate`/`push-dispatch` — nenhuma sobreposição, sem necessidade de `BLOCKERS.md`). `supabase/functions/voice-capture/`: `lib.ts` (validação de entrada — `transcript` com prioridade sobre `audio_base64`/`mime_type` de fallback, mesmo limite de 8MB/MIME de áudio de `receipt-ocr` — e heurísticas de linguagem natural pt-BR: valor via símbolo `R$`, "`X` reais (e `Y` centavos)" numérico, ou numeral por extenso — "cinquenta reais", "cento e vinte reais", "dois mil reais" — data via "hoje"/"ontem"/"anteontem"/`dd/mm(/aaaa)`/"dia N", com default "hoje" de confiança reduzida quando nada é reconhecido — decisão de interpretação pequena documentada no próprio código, já que captura por voz é majoritariamente sobre gasto recém-ocorrido —, tipo entrada/saída por verbo, sugestão de categoria por palavra-chave incluindo termos de voz como "uber"/"ifood", e descrição livre com verbo/valor/marcador de tempo removidos), `index.ts` (wiring HTTP: exige JWT de sessão, mesmo padrão de `/webauthn-register`/`/receipt-ocr`; nunca persiste nada; mapeia entrada inválida para `400`, ausência de sessão para `401`, fallback de STT em nuvem não configurado para `503 stt_fallback_not_configured`, falha do vendor de fallback para `502 stt_provider_failed`). `_shared/sttProvider.ts` (novo): interface `STTProvider` (DIR-22, mesmo espírito de `OCRProvider`) documentando o ponto de extensão do fallback em nuvem — `getConfiguredSTTProvider()` retorna `null` deliberadamente, pois ADR-006 decide esse fallback como "extensão futura, não dependência obrigatória do início da Fase 3", sem vendor escolhido (diferente de OCR, que teve SPK-002 resolvendo o vendor); nenhum adapter concreto foi criado para não reabrir sozinho essa decisão de arquitetura. Resultado sempre inclui `suggestion_disclaimer = "sugestão automática, não confirmada"` (AC1, texto literal), além da semântica já implícita de `{ value, confidence }` por campo e do fluxo de confirmação humana de `candidate_transaction` (`BE-F3-00`). Contrato publicado em `API-CONTRACT.yaml` v0.22.0 (`/voice-capture`). **Teste automatizado** `lib.test.ts`: 27/27 casos PASS (`deno check`/`deno lint` limpos) — cobre (a) transcrição bem-formada extrai valor/data/tipo/categoria corretamente com `suggestion_disclaimer` presente; (b) campo não reconhecido (valor ausente, verbo de tipo ausente, categoria sem palavra-chave, transcrição vazia) retorna `undefined` sem lançar e sem afetar os demais, incluindo o caso de data implausível (mês 13) que descarta o candidato explícito sem lançar; (c) validação de entrada (`transcript` vs. `audio_base64`, tamanho máximo, MIME de áudio, base64 inválido) nunca lança. Regressão completa da suíte `deno test supabase/functions/`: 94/94 casos PASS (47 pré-existentes + 20 de `receipt-ocr` + 27 novos deste lote), 0 falhas. **Ressalva de transparência (mesmo padrão de BE-F3-01)**: cobertura de parsing de numerais por extenso em pt-BR é heurística própria do produto, suficiente para valores falados do dia a dia (unidades/dezenas/centenas + "mil"), mas não cobre milhões nem frações incomuns — aceitável para este produto, documentado em `lib.ts`; nenhum smoke test empírico com a Web Speech API real de um navegador foi rodado nesta sessão (function só recebe o `transcript` já produzido pelo client, então a integração real com a Web Speech API em si é escopo de `FE-F3-02`, não desta tarefa) | Captura Automatizada — Voz & Foto |
| BE-F3-03 | Parser de extrato OFX/CSV (Edge Function) + detecção de possível duplicata (mesma data/valor/conta) | Backend | RF-F3-03 AC1-2 | Transação candidata coincidente com lançamento existente é sinalizada antes da confirmação (AC2) | 2 dias | **Concluída — 2026-09-08.** `DIR-33` checado antes de escrever código: `supabase functions list` no projeto linkado não mostrou nenhuma function de importação de extrato pré-existente (mesmas 8 já conhecidas de `BE-F3-01`/`02` — `auth-email-mfa`/`webauthn-*`/`backup-export`/`invoice-close`/`recurring-generate`/`fixed-bill-generate`/`push-dispatch` — nenhuma sobreposição, sem necessidade de `BLOCKERS.md`); `supabase secrets list` também conferida, sem secret relevante pré-existente para este escopo. `supabase/functions/statement-import/`: `lib.ts` (parser OFX via extração de blocos `<STMTTRN>` por regex — não um parser XML/SGML estrito, decisão documentada no código: extratos OFX reais frequentemente têm tags não fechadas que um parser XML rejeitaria —, `DTPOSTED`/`TRNAMT`/`MEMO`\|`NAME`/`FITID`; parser CSV por nome de coluna reconhecido em pt-BR/en — `data`\|`date`, `valor`\|`amount`\|`value`, `descricao`\|`description`\|`historico`\|`memo` —, delimitador `,`\|`;` autodetectado pelo cabeçalho, datas `dd/mm/aaaa`\|ISO, valor BRL com milhar/decimal por vírgula; `findDuplicateMatch` — função pura, mesma `transaction_date`+`amount_cents` de um lançamento existente já filtrado por `account_id` pelo chamador), `index.ts` (wiring HTTP: exige JWT de sessão, mesmo padrão de `/receipt-ocr`/`/voice-capture`; única leitura ao banco é `SELECT id, transaction_date, amount_cents FROM transactions WHERE account_id = :account_id AND transaction_date BETWEEN :min AND :max` via `userClient` — RLS aplicada, nunca `service_role` —, usada só para a checagem de duplicata; falha nessa query específica degrada para "nenhuma duplicata sinalizada" nesta resposta, logada como erro, sem derrubar a apresentação da lista). **Decisão de design central (AC1/AC3, mesmo princípio já estabelecido por `BE-F3-01`/`BE-F3-02`)**: esta Edge Function **não persiste nada** em `import_batch` nem `candidate_transaction` — é pura interpretação de arquivo + checagem de duplicata (só leitura), devolvendo a lista já no formato de `NewCandidateTransaction` (`source=import`, `BE-F3-00`) pronta para o Frontend (`FE-F3-05`, ainda não iniciada) persistir via `POST /import_batch` + `POST /candidate_transaction` depois que o usuário revisar/selecionar a lista — reuso explícito do mesmo fluxo de candidatos/revisão de voz e foto (FL-05), conforme instrução explícita desta tarefa, não um mecanismo de persistência paralelo. Efeito colateral relevante desta escolha: como nenhum `candidate_transaction.import_batch_id` é gravado por esta function, o achado `SEC-DEBT-015`/condição de bloqueio registrada em `SECURITY-REVIEW.md` Seção "Achados" (FK `import_batch_id` com `ON DELETE CASCADE` divergente do contrato) continua **inexploitável** depois desta tarefa também — ela só passa a ser exercitada de fato quando `FE-F3-05` começar a chamar `POST /import_batch`/`POST /candidate_transaction` com `import_batch_id` preenchido; `BE-DEBT-04` continua pendente, sem regressão introduzida aqui, e a condição de bloqueio antes de produção permanece válida para quando o consumo real (`FE-F3-05`) existir. Contrato publicado em `API-CONTRACT.yaml` v0.23.0 (`/statement-import`). **Teste automatizado** `lib.test.ts`: 23/23 casos PASS (`deno check`/`deno lint` limpos) — cobre (a) parsing de OFX válido com transação de entrada e saída, valor/data/descrição/`external_ref` corretos; (b) parsing de CSV válido com cabeçalho pt-BR (delimitador `;`, valor BRL) e com cabeçalho en (delimitador `,`, data ISO); (c) fallback gracioso sem crash — bloco OFX sem `DTPOSTED`/`TRNAMT` reconhecível é pulado (`skipped_lines`) sem afetar os demais, linha de CSV com data ou valor inválido idem, arquivo totalmente ilegível (sem `<STMTTRN>` nenhum, ou CSV sem cabeçalho reconhecível, ou arquivo vazio) retorna lista vazia sem lançar — tratado por `index.ts` como `422 unprocessable_file`; (d) `findDuplicateMatch` sinaliza corretamente quando `transaction_date`+`amount_cents` coincidem com um lançamento existente, não sinaliza quando um dos dois difere, e retorna `undefined` sem lançar quando não há lançamentos existentes; além de validação de entrada (`file_content_base64`/`file_format`/`account_id` — incluindo formato de uuid — e limite de 5MB) nunca lançando. Regressão completa da suíte `deno test supabase/functions/`: 117/117 casos PASS (94 pré-existentes + 23 novos deste lote), 0 falhas. **Achado/decisão residual não-bloqueante**: parser CSV cobre um layout de "uma coluna de valor assinado" (convenção comum de exportação bancária, negativo = saída) — não cobre o layout alternativo "colunas separadas de débito/crédito" que alguns bancos usam; decisão de escopo pequena, documentada em `lib.ts`, ajustável quando um caso real de banco brasileiro específico exigir (nenhuma amostra real de extrato foi usada nesta sessão, mesma ressalva de transparência já registrada em `BE-F3-01`/`BE-F3-02` para os respectivos vendors/APIs). | Captura Automatizada — Importação de Extrato |
...
| BE-F3-06 | Query de evolução patrimonial (série temporal do saldo consolidado, filtrável por conta) | Backend | RF-F3-05 AC1-2 | Filtro por conta individual retorna série coerente com a visão consolidada | 1 dia | **Concluída — 2026-09-09.** RPC `public.get_net_worth_evolution(p_account_id uuid default null)` (migration `20260909100000_be_f3_06_net_worth_evolution.sql`, aplicada via `supabase db push --linked`, par remoto confirmado via `migration list --linked`) — sem tabela nova, sem Edge Function (DIR-33 não se aplica), reaproveitando `accounts.current_balance_cents` (mesma fonte de `get_month_provision`/BE-M-07) e o desenho de janela/granularidade já estabelecido por `get_income_expense_report`/BE-F2-10 (mês corrente + 5 anteriores, America/Sao_Paulo — decisão de interpretação pequena, documentada na própria migration, já que RF-F3-05 AC1-2 não fixa janela). Cada ponto reconstrói o saldo ao final do mês revertendo (mesmo sinal de `apply_transaction_effect`) o efeito de toda transação datada depois do fim daquele mês; `p_account_id` (AC2) restringe a soma a 1 conta, conta alheia/inativa produz lista vazia (nunca erro/vazamento). Contrato publicado em `API-CONTRACT.yaml` v0.24.0 (`/rpc/get_net_worth_evolution`). **Teste automatizado** `supabase/tests/be_f3_06_net_worth_evolution.test.sql` via RLS real (`SET LOCAL ROLE authenticated`, `BEGIN`/`ROLLBACK`) — casos A-F: deltas corretos por conta após lançamentos em 2 contas de teste; **coerência AC2 literal** (soma da série de cada conta individual bate ponto a ponto com a série consolidada em toda a janela de 6 meses, inclusive após lançamento fora da janela); janela de 6 meses exclui lançamento de 7 meses atrás; nunca mais de 6 linhas; isolamento cross-user (conta de outro usuário como filtro retorna lista vazia, não erro) — todos PASS. Regressão completa da suíte SQL: 31/32 PASS — a única falha (`be_m07_dashboard.test.sql` CASO 2) é pré-existente e não relacionada a esta tarefa (`BLOCKERS.md` Bloqueio 019, dado real de produção). | Relatórios & Exportação (Fase 3) |
| BE-F3-07 | Exportação CSV/PDF (Edge Function ou geração client-side com `pdf-lib`) | Backend | RF-F3-06 AC1-2 | CSV contém no mínimo data, conta, forma de pagamento, categoria, subcategoria, descrição, tipo, valor (AC1); PDF contém resumo do período (saldo, entradas, saídas, distribuição por categoria — layout mínimo definido na Seção 6) | 2 dias | **Concluída — 2026-09-09.** `DIR-33` checado antes de escrever código: `supabase functions list` no projeto linkado não mostrou nenhuma function de exportação/relatório pré-existente (mesmas 8 já conhecidas de `BE-F3-01`/`02`/`03`/`06` — nenhuma sobreposição, sem necessidade de `BLOCKERS.md`). Duas peças, ambas 100% aditivas (migration `20260909110000_be_f3_07_report_export.sql`, aplicada via `supabase db push --linked`, par remoto confirmado via `migration list --linked`): (1) RPC `public.get_report_export_rows(p_start_date, p_end_date)` (SECURITY INVOKER, escopada por `auth.uid()`, mesma família de `get_income_expense_report`/`get_net_worth_evolution`) devolvendo cada lançamento do período já com conta/forma de pagamento/categoria/subcategoria resolvidos — `category_name` é sempre a categoria de TOPO e `subcategory_name` só quando o lançamento usa uma subcategoria de 2º nível (nunca invertidos, decisão de interpretação pequena documentada na migration), inclui `kind=transfer` (extrato completo do período, diferente do comparativo de `get_income_expense_report`, que o exclui); (2) bucket privado de Storage `exports` + policies de `storage.objects` restringindo cada usuário à própria pasta (`<user_id>/...`), mesmo princípio de isolamento de `G-18`/SDD.md Seção 7 — bucket que a futura `BE-F3-08` (expurgo de export >24h) vai precisar existir, sem tabela de metadado nova (o `created_at` do próprio objeto no Storage já é suficiente para aquele job filtrar por idade). Edge Function `supabase/functions/report-export/`: `lib.ts` (validação de entrada — `start_date`/`end_date` aaaa-mm-dd com validação de calendário plausível, `format` csv/pdf —, `buildCsvContent` montando o CSV com as 8 colunas literais do AC1, delimitador `;`, valor com sinal formatado em BRL, escapamento RFC 4180; `computeReportSummary` agregando entradas/saídas/saldo/distribuição por categoria para o PDF, excluindo `kind=transfer` do resumo — mesmo critério de `get_income_expense_report`/BE-F2-10, decisão documentada em `lib.ts`), `pdf.ts` (`npm:pdf-lib`, 1 página A4 com título/período/resumo/distribuição por categoria — layout mínimo próprio do produto, já que nem RF-F3-06 nem UX-SPEC.md detalham um mockup pixel-a-pixel para o CONTEÚDO do PDF em si, decisão de interpretação pequena documentada em `pdf.ts`, mesmo padrão de decisão de layout físico já tomado por `BE-F3-06`/`BE-F2-10`), `index.ts` (wiring HTTP: exige JWT de sessão, mesmo padrão de `/receipt-ocr`/`/voice-capture`/`/statement-import`; lê via `userClient`/RLS, nunca `service_role`; faz upload do arquivo gerado ao bucket `exports` via o próprio `userClient` — respeitando a policy de isolamento por pasta, sem precisar de service role — e devolve uma signed URL de 300s, nunca uma URL pública; nunca finge sucesso — falha de consulta/geração/upload/assinatura sempre retorna erro HTTP claro, nenhum download parcial). Contrato publicado em `API-CONTRACT.yaml` v0.25.0 (`/rpc/get_report_export_rows`, `/report-export`). **Teste automatizado SQL** `supabase/tests/be_f3_07_report_export.test.sql` via RLS real (`SET LOCAL ROLE authenticated`, `BEGIN`/`ROLLBACK`) — casos A-F: categoria de topo sem subcategoria, subcategoria mapeada corretamente para a categoria-pai (AC1 literal), `transfer` incluído no extrato completo, filtro de período excluindo lançamento fora da janela, isolamento cross-user, período vazio retornando lista vazia sem erro — todos PASS. **Teste automatizado Deno**: `lib.test.ts` 28/28 PASS (validação de entrada, `formatCentsToBrl`/`formatDateBr`/`kindLabel`, montagem do CSV incluindo cabeçalho/linha vazia/escapamento, agregação do resumo incluindo exclusão de `transfer` e ordenação por valor absoluto) + `pdf.test.ts` 4/4 PASS (confirma, recarregando os bytes gerados com o próprio `pdf-lib` e descomprimindo os content streams `FlateDecode` via `DecompressionStream`, que o PDF contém literalmente "Saldo do período"/"Entradas"/"Saídas"/"Distribuição por categoria" e os valores esperados — AC2 literal). Regressão completa: `deno test supabase/functions/` 149/149 PASS (117 pré-existentes + 32 novos); `deno check`/`deno lint` limpos; suíte SQL completa 33/34 PASS — a única falha (`be_m07_dashboard.test.sql` CASO 2) é pré-existente e não relacionada a este lote (`BLOCKERS.md` Bloqueio 019, dado real de produção). **Decisão de escopo documentada (não é lacuna bloqueante)**: sem filtro de conta em `/report-export` — `UX-SPEC.md` S-REP-03 só define período + formato (CSV/PDF, radio) + botão "Exportar", sem seleção de conta; e sem smoke test empírico de abrir o PDF/CSV gerado num leitor real (Excel/Acrobat) nesta sessão — a corretude do conteúdo foi provada por inspeção estrutural dos bytes (mesmo padrão de ressalva de transparência já usado por `BE-F3-01`/`02`), não por verificação visual humana. | Relatórios & Exportação (Fase 3) |
| BE-F3-08 | **[Reestimada]** Jobs diários de expurgo de dado transitório conforme ADR-011: `CandidateTransaction` descartado/abandonado (30 dias) + foto de recibo associada (30 dias); foto de recibo de lançamento confirmado (90 dias após `confirmed_at`); export CSV/PDF (24h após geração) | Backend | CC-01 (Seção 6, resolvida), `adr/011-politica-retencao-descarte-dado-exclusao-conta.md` | Candidato com `status = descartado` há mais de 30 dias (ou `pendente` sem ação há mais de 30 dias desde a criação do `ImportBatch`) é removido pelo job diário junto da foto associada no Storage; foto de recibo de lançamento confirmado é removida 90 dias após `confirmed_at`, sem afetar o `Transaction`; export gerado há mais de 24h é removido do bucket de exports; falha de execução de qualquer um dos três jobs gera log/alerta consultável (mesmo padrão de DIR-32) | 2.5 dias | **Concluída — 2026-09-09.** Bloqueio 024 (`BLOCKERS.md`, Resolvido): o Software Architect formalizou, via nova `ADR-020` (`adr/020-foto-de-recibo-nunca-persistida-linhas-de-retencao-do-adr-011-sem-objeto.md`), que foto de recibo NUNCA será persistida (decisão definitiva do stakeholder) — as 2 linhas de `ADR-011` sobre retenção de foto (90 dias/lançamento confirmado; 30 dias/candidato descartado) ficam sem objeto por decisão de produto, não por lacuna de implementação; `SDD.md` Seção 5.2 (`Attachment`) e Seção 7 já refletem isso. Consequência direta nesta tarefa: o 3º sub-job (`confirmed_receipt_photo_purge`, que rodava como skip registrado `status: skipped_not_implemented` enquanto o bloqueio estava aberto) foi removido por completo do código (`supabase/functions/data-retention-purge/index.ts`/`lib.ts` — não existe mais nem como skip) e a menção a "foto associada" foi removida do comentário de `purge_expired_candidate_transactions` (nova migration aditiva, comentário apenas, `20260909122000_be_f3_08_adr020_drop_photo_comment.sql` + down). Os 2 sub-jobs restantes cobrem 100% do que de fato precisa ser expurgado: (1) `candidate_transaction_purge` — remove `CandidateTransaction` descartado >30 dias ou `pending` abandonado >30 dias desde a criação do `ImportBatch` associado (ou do próprio `created_at`, para candidato avulso de voz/foto sem lote — decisão de interpretação pequena já documentada na migration original); (2) `export_purge` — remove objeto do bucket `exports`/BE-F3-07 criado >24h. Migrations `20260909120000_be_f3_08_data_retention_purge_jobs.sql` + `20260909121000_be_f3_08_revoke_execute_purge_functions.sql` já estavam aplicadas via `supabase db push --linked` (confirmadas via `migration list --linked`) desde a rodada anterior; a nova migration `20260909122000` foi escrita e regressão-testada localmente (`deno check`/`deno lint`/`deno test` limpos), mas a aplicação via `supabase db push --linked`/`db query --linked --file` foi bloqueada nesta sessão pelo classificador de permissão do ambiente do Executor — pendência puramente operacional (aplicar a migration), não uma lacuna de especificação ou implementação; o comentário desatualizado em si não afeta comportamento (COMMENT ON FUNCTION é metadado, não lógica). Contrato atualizado em `API-CONTRACT.yaml` v0.27.0 (`/data-retention-purge`: `job_name`/`status` sem `confirmed_receipt_photo_purge`/`skipped_not_implemented`). **Teste automatizado**: `supabase/functions/data-retention-purge/lib.test.ts` 8/8 PASS (`overallStatus` ajustado para os 2 sub-jobs reais, sem o caso `skipped`); `supabase/tests/be_f3_08_data_retention_purge_jobs.test.sql` 7/7 PASS via `supabase db query --linked --file` (os 7 casos já cobriam só os 2 sub-jobs reais — nenhuma mudança de caso necessária). Regressão completa: `deno test supabase/functions/` 157/157 PASS (mesma contagem da rodada anterior — 1 teste ajustado, nenhum adicionado/removido); `deno check`/`deno lint` limpos. | Retenção & Descarte de Dado / Exclusão de Conta |
| BE-F3-09 | **[Nova — ADR-011]** Edge Function de exclusão de conta a pedido do usuário (role de serviço, nunca exposta como operação direta do cliente) | Backend | ADR-011 (tabela-resumo, linha "Exclusão de conta"), CC-01 (Seção 6, resolvida) | Ação autenticada e explícita do usuário dispara a Edge Function, que remove todas as linhas de `public` associadas ao `user_id` (respeitando dependência de FK/cascade), todos os objetos do Storage do mesmo `user_id` (fotos de recibo, exports pendentes), e o usuário correspondente em Supabase Auth; chamada sem o JWT do próprio usuário-alvo é rejeitada | 2 dias | **Concluída — 2026-09-15, com ressalva documentada.** Nova Edge Function `supabase/functions/delete-account/` (`index.ts`/`lib.ts`), nova função SQL `SECURITY DEFINER` `public.delete_user_data(p_user_id uuid)` (migration `20260915090000_be_f3_09_delete_account_data.sql` + down) — `EXECUTE` restrito a `service_role` (`GRANT`/`REVOKE` explícito), nunca chamável diretamente pelo client mesmo autenticado, cumprindo literalmente "role de serviço, nunca exposta como operação direta do cliente" do ADR-011. Orquestra, nesta ordem: (i) `delete_user_data` remove, numa única transação atômica, toda linha de `public` associada ao usuário nas 19 tabelas "ownable" + `profiles` hoje existentes (`accounts`/`budget`/`categories`/`payment_methods`/`transactions`/`profiles`/`webauthn_credentials`/`webauthn_challenges`/`email_mfa_challenges`/`credit_cards`/`invoices`/`recurring_templates`/`recurring_template_adjustments`/`installment_purchases`/`fixed_bills`/`goals`/`contributions`/`notifications`/`push_subscriptions`/`candidate_transaction`/`import_batch`), numa ordem que respeita todo `FK RESTRICT` e os triggers RN-08/RN-09 (`accounts_block_delete_when_linked`/`categories_block_delete_when_linked`) — documentada linha a linha no cabeçalho da migration (ex.: `fixed_bills`/`installment_purchases`/`recurring_templates` têm FK `RESTRICT` para `category_id`/`account_id`/`payment_method_id` e precisam ser apagadas antes dessas 3 tabelas; `transactions` precisa vir antes de `accounts`/`categories`/`invoices`; `budget` antes de `categories`; `categories` em 2 passadas por causa da auto-referência `parent_category_id`); falha em qualquer `DELETE` reverte tudo (nada de exclusão parcial silenciosa). Confiar só no `ON DELETE CASCADE` já presente em toda tabela "ownable" (auditado e confirmado em todas as migrations) foi descartado deliberadamente — a ordem em que o Postgres processaria múltiplos cascades independentes vindos do mesmo `DELETE FROM auth.users` não é garantida, e os triggers RN-08/RN-09 poderiam bloquear a operação inteira se a ordem "errada" fosse tentada; (ii) exports pendentes do usuário no bucket `exports` (BE-F3-07) removidos via Storage API — falha aqui é logada como aviso mas não bloqueia o passo seguinte (dado financeiro já removido no passo i; um export órfão é bloat, não vazamento, bucket privado sem policy de acesso para usuário que deixará de existir); (iii) usuário removido de Supabase Auth via Admin API do GoTrue (`auth.admin.deleteUser`, nunca `DELETE FROM auth.users` via SQL puro — a Admin API cuida da limpeza correta das demais tabelas do schema `auth`). Falha no passo (iii) é tratada como estado crítico/parcial, nunca mascarada como sucesso (resposta `500 auth_account_deletion_failed`, `partial:true`). Guardrail central: corpo da requisição é opcional; quando ausente, o alvo é sempre o próprio usuário autenticado (nunca precisa ser informado); quando `user_id` é informado, DEVE bater com o `sub` do JWT — qualquer divergência é rejeitada com `403 forbidden_target_mismatch` (`lib.ts#validateTargetUserId`), cumprindo literalmente o critério de aceite "chamada sem o JWT do próprio usuário-alvo é rejeitada". Fotos de recibo: SEM OBJETO (`ADR-020`, mesmo tratamento já dado por `BE-F3-08`) — nenhuma foto é persistida em nenhum caminho do sistema, nada a remover além de exports. Reautenticação para ação sensível (referenciada em TASK.md linha 156-157): o mecanismo é a própria exigência de JWT de sessão válido do usuário-alvo — `SDD.md` Seção 7/`ADR-014` confirmam que o claim `app_email_mfa_verified` é sempre `'true'` por decisão definitiva do stakeholder (sem gate real), então checar esse claim aqui seria decorativo, não proteção real; um fluxo de confirmação de UI dedicado é responsabilidade do Frontend (`FE-F3-09`), fora do escopo desta tarefa de Backend. **Ressalva documentada (não é lacuna real hoje, ver dependência de contrato parcial da Seção 4.3)**: `BE-F3-04` (Open Finance) ainda não foi implementada — não existe hoje nenhuma tabela real de conexão/token Open Finance no schema `public` para esta function cobrir; quando `BE-F3-04` criar essa tabela (mesmo padrão `user_id ... references auth.users on delete cascade` de toda tabela "ownable"), `delete_user_data` precisará ganhar um novo `DELETE` para ela — documentado explicitamente no cabeçalho da própria migration, mesmo padrão de lacuna futura já usado por `BE-F3-08` para foto de recibo. Contrato publicado em `API-CONTRACT.yaml` v0.28.0 (`/delete-account`). **Teste automatizado**: `supabase/functions/delete-account/lib.test.ts` 11/11 PASS (`validateTargetUserId` cobre o critério de aceite (d) diretamente — aceita corpo sem `user_id`/com `user_id` igual, rejeita `user_id` de outro usuário e tipos inválidos; `buildExportObjectPaths`/`chunk`/`totalDeletedRows` cobertos); `supabase/tests/be_f3_09_delete_account_data.test.sql` (5 casos, escrito e pronto — fixture real em TODAS as 20 tabelas cobertas para um usuário-alvo E um usuário de controle, chama `delete_user_data` real, confirma 0 linhas restantes do alvo em todas as tabelas E as 20 linhas do usuário de controle intactas, mais idempotência e rejeição de `p_user_id NULL`) — **validado localmente via `deno`/lógica, mas a execução real via `supabase db query --linked --file` não foi feita nesta sessão**: ambiente do Executor sem CLI do Supabase instalado nem credencial do projeto linkado disponível — mesma pendência puramente operacional (não uma lacuna de especificação/implementação) já registrada por `BE-F3-08` em rodada anterior para `supabase db push --linked`; o SQL do teste está pronto para quem tiver a CLI/credencial rodar. Regressão completa: `deno test supabase/functions/` 168/168 PASS (157 anteriores + 11 novos, nenhuma quebra); `deno check`/`deno lint` limpos em `supabase/functions/delete-account/`. | Retenção & Descarte de Dado / Exclusão de Conta |
| BE-F3-10 | **[Nova — ADR-011]** Rotação de backup: expurgo do snapshot mais antigo a cada execução, mantendo no máximo os últimos 30 snapshots diários (extensão da Edge Function de `BE-M-10`) | Backend | ADR-011 (tabela-resumo, linha "Backup/exportação lógica") | A cada execução diária do job de backup, se já houver 30 snapshots armazenados, o snapshot mais antigo é removido, mantendo o total em no máximo 30 | 0.5 dia | **Concluída — 2026-09-15.** Investigação prévia (mesmo espírito de `DIR-33`, aplicado aqui à própria Edge Function em vez de auditoria de listagem): a rotação exigida por este critério de aceite — "se já houver 30 snapshots, remove o mais antigo, mantendo o total em no máximo 30" — **já foi entregue por completo dentro de `BE-M-10`**, não é trabalho novo. `supabase/functions/backup-export/lib.ts` já define `objectsToDelete(keysNewestFirst, keep = 30)` (mantém só os `keep` mais novos, retorna o excedente a apagar) e `sortKeysNewestFirst`; `index.ts` (passo 4, comentário literal "Rotação — mantém só os últimos 30 snapshots diários (ADR-011)") já lista os objetos do bucket via `list-type=2&prefix=mymoney-backups/`, ordena por chave (particionada por data ISO, lexicograficamente ordenável) e deleta cada chave excedente a cada execução do modo `export` — exatamente o comportamento pedido por este critério de aceite, já vinculado ao ADR-011 desde o commit original de `BE-M-10`. Não havia "rotação de outra coisa" (ex.: chave de criptografia) para confundir com este escopo — a nota de status de `BE-M-10` na Seção 3.1 e o comentário de topo de `index.ts` referem-se à mesma rotação de quantidade de snapshots, não a rotação de segredo. Nenhuma linha de código nova foi necessária; nenhuma duplicação de lógica foi introduzida. **Confirmação, não invenção de teste novo**: `supabase/functions/backup-export/lib.test.ts` já continha, desde `BE-M-10`, os dois casos exatos exigidos pelo critério de aceite desta tarefa — "objectsToDelete mantém só os 30 mais novos, retorna o excedente (ADR-011, rotação)" (35 chaves simuladas → 5 removidas, exatamente `k30`..`k34`) e "objectsToDelete com <= 30 chaves não apaga nada" (10 chaves simuladas → nenhuma removida) — cobrindo os dois sentidos da fronteira (>30 remove o mais antigo repetidamente até caber em 30; <=30 não remove nada). Regressão revalidada nesta sessão: `deno test supabase/functions/backup-export/lib.test.ts` → 16/16 PASS, sem resíduo. **Dependência real, não-bloqueante**: a rotação em si roda sempre dentro do passo "storage externo" do job (`index.ts`, try/catch dedicado, best-effort — falha na rotação nunca reverte o sucesso do upload do dia), mas a execução de ponta a ponta contra um bucket S3-compatível real ainda depende das credenciais externas (`BACKUP_S3_*`) não configuradas nesta sessão (`BLOCKERS.md` Bloqueio 007, `Aberto`) — mesma dependência não-bloqueante já registrada por `BE-M-10`; a lógica pura de contagem/expurgo (o que este critério de aceite pede) é 100% testável e testada sem depender de credencial real, mesmo padrão já aceito em `BE-M-10`. `API-CONTRACT.yaml` conferido — nenhuma mudança de contrato necessária (a rotação é um efeito colateral interno do modo `export` de `/backup-export`, já documentado desde `BE-M-10`; não altera request/response). | Retenção & Descarte de Dado / Exclusão de Conta |
...
| FE-F3-02 | Captura por voz: S-CAP-02 (`VoiceRecorderUI`, transcrição interina ao vivo, `aria-live`) | Frontend | UX-FL-04, S-CAP-02, DIR-15 | Estado "Ouvindo..." e transcrição interina são anunciados via `aria-live`, não só exibidos visualmente | 2 dias | **Concluída — 2026-09-07.** `frontend/src/lib/speechRecognition.ts` estendido (além de `hasSpeechRecognitionSupport` de `FE-F3-01`) com `createSpeechRecognizer()`: wrapper real de `SpeechRecognition`/`webkitSpeechRecognition` (pt-BR, `continuous`/`interimResults` ligados), com tipos locais mínimos (a API não faz parte de `lib.dom.d.ts`), acumulando resultados finais e repassando interinos por chamada, e normalizando `onerror` para um conjunto fechado de códigos (`no-speech`/`audio-capture`/`not-allowed`/`network`/`aborted`/`unknown`) — nunca deixa o evento nativo vazar como exceção não tratada. Novo componente `VoiceRecorderUI` (`frontend/src/components/domain/VoiceRecorderUI.tsx`), aberto por `CaptureFab` ao selecionar "Falar" (troca de conteúdo do mesmo `Modal`/`BottomSheet`, mesmo padrão de `ReceiptCameraCapture`/`FE-F3-03`) — mic pulsante (`animate-pulse`) + "Concluir gravação"/"Cancelar". **AC literal**: o estado "Ouvindo..." e a transcrição interina vivem dentro do **mesmo** contêiner visível `role="status" aria-live="polite"` (não `sr-only` — é ao mesmo tempo a exibição visual e a região anunciada), confirmado por teste automatizado que localiza o elemento via `closest('[aria-live]')` e checa o atributo e o conteúdo de texto explicitamente, não presume. `"polite"` escolhido (não `"assertive"`) porque o usuário está no meio de uma fala contínua — interromper a cada palavra interina seria mais prejudicial que aguardar a próxima pausa natural do leitor de tela; erros (que exigem atenção mais imediata) usam o `Alert` do design system, que já aplica `role="alert"`/`aria-live="assertive"` por padrão. Erros de reconhecimento (`onerror`) e de extração (`/voice-capture` fora do ar) nunca lançam exceção nem travam a tela — sempre saem em "Tentar novamente" (reinicia a gravação do zero) ou "Cancelar" (fecha o modal, volta ao menu de 3 opções — "usar foto/manual em vez disso" do UX-SPEC), incluindo o caso defensivo de navegador sem suporte (`CaptureFab` já filtra isso a montante, mas `VoiceRecorderUI` trata com segurança caso seja usado fora desse contexto) e o caso de "Concluir gravação" sem nenhuma fala reconhecida (erro local, sem sequer chamar a API). **Decisão de escopo (documentada aqui, conforme instrução desta tarefa)**: como `/voice-capture` (`BE-F3-02`) já está publicado em `API-CONTRACT.yaml` v0.22.0, não persiste nada, e o AC desta tarefa é sobre a UI de gravação em si (S-CAP-02) — "Concluir gravação" chama o endpoint **real** (`extractVoiceCapture`, novo `frontend/src/lib/api/voiceCapture.ts`, via `invokeEdgeFunction` já usado por `webauthn-*`) com a transcrição finalizada, sem mock intermediário; o resultado extraído é repassado a `CaptureFab` (que hoje só fecha o modal e mostra aviso "em breve" — o `DraftReviewBanner` que exibe esse resultado é `FE-F3-04`, ainda não iniciada). Nenhuma pendência de mock nesta tarefa (por isso fecha como `Concluída`, não `Em andamento`). `CaptureFab.tsx` atualizado: "Falar" habilitada agora abre `VoiceRecorderUI` de fato (era aviso "em breve"). Testes novos: `VoiceRecorderUI.test.tsx` (10 casos — início automático da gravação, aria-live do estado "Ouvindo...", aria-live da transcrição interina, conclusão bem-sucedida chamando `/voice-capture` com a transcrição final e repassando o resultado, estado "Interpretando..." com aria-live durante a chamada, cancelar parando o reconhecimento, erro de permissão negada com "Tentar novamente"/"Cancelar", retry reiniciando uma nova instância de reconhecimento, "Concluir" sem fala reconhecida sem chamar a API, falha da API mostrando erro recuperável, navegador sem suporte) e `CaptureFab.test.tsx` atualizado (teste antigo de "Falar fecha o menu" substituído por um que confirma a troca para `VoiceRecorderUI`, com mock de `SpeechRecognition` agora incluindo `start`/`stop`). Suíte completa: `npx vitest run` — 61 arquivos, 352 testes, todos passando; `npx tsc -b` sem erros; `npx oxlint` sem erros novos (só os mesmos warnings pré-existentes `react(set-state-in-effect)`/`react(only-export-components)` já presentes em outras ~14 páginas/arquivos do projeto). | Captura Automatizada — Voz & Foto |
| FE-F3-03 | Captura por foto: S-CAP-04 (`ReceiptCameraCapture`, moldura-guia, upload alternativo, pré-visualização) | Frontend | UX-FL-04, S-CAP-04 | Permissão de câmera negada oferece upload de arquivo como alternativa, nunca bloqueia o usuário (Seção 4.2 UX-SPEC) | 2 dias | **Concluída — 2026-09-07.** Novo componente `ReceiptCameraCapture` (`frontend/src/components/domain/ReceiptCameraCapture.tsx`), aberto por `CaptureFab` (`FE-F3-01`) ao selecionar "Fotografar" — troca o conteúdo do mesmo `Modal`/`BottomSheet` (`FE-M-01`) já usado pelo menu, mantendo foco/teclado/Esc herdados. Suporte de câmera detectado por `hasCameraCaptureSupport()` (`frontend/src/lib/receiptImage.ts`, checa só `navigator.mediaDevices.getUserMedia`, mesmo princípio de `hasSpeechRecognitionSupport` de `FE-F3-01`); `getUserMedia` é então chamado e qualquer resultado que não seja sucesso (API ausente, permissão negada, ou qualquer outra falha do dispositivo) cai no mesmo estado `unavailable` — critério de aceite literal: o upload de arquivo (`<input type="file" accept="image/*">`, associado a um `<label>` visível "Selecionar arquivo") é renderizado **incondicionalmente** desde o primeiro render (estado `checking`), nunca escondido atrás de um erro. Quando a câmera está disponível: viewfinder (`<video>`) com moldura-guia sobreposta (`div` tracejada + texto "Posicione o recibo dentro da moldura", ambos `aria-hidden`/decorativos) e botão "Capturar foto" lado a lado com o upload. Tanto a captura por câmera (`captureVideoFrame`, canvas) quanto o upload (`readFileAsDataUrl`, `FileReader`) convergem para a mesma tela de pré-visução ("Usar esta foto" / "Tirar novamente") antes de qualquer confirmação — nada é enviado sem essa etapa. "Usar esta foto" roda `validatePreparedImage` (espelho client-side de `MAX_IMAGE_BYTES`/`ALLOWED_MIME_TYPES` de `supabase/functions/receipt-ocr/lib.ts`, `BE-F3-01`) antes de chamar `onConfirm`; falha de validação mostra `Alert` inline e mantém a pré-visualização, sem perder a foto já capturada/selecionada. Stream de câmera sempre é parado (`track.stop()`) ao confirmar, cancelar ou desmontar. Escopo desta tarefa termina na imagem preparada (base64 + `mimeType`) — envio real a `/receipt-ocr` e o `DraftReviewBanner` são `FE-F3-04` (ainda não iniciada); `CaptureFab` hoje só fecha o modal e mostra um aviso "em breve" ao confirmar a foto, decisão de escopo documentada no próprio componente. Testes novos: `ReceiptCameraCapture.test.tsx` (câmera ausente, permissão negada, câmera concedida + captura + retake + confirmação + validação client-side falhando + cancelamento parando o stream — `captureVideoFrame` mockado via `vi.mock` parcial, já que jsdom não implementa canvas 2D) e `CaptureFab.test.tsx` atualizado (teste antigo de "Fotografar fecha o menu" substituído por dois novos: abre `ReceiptCameraCapture` de fato, e "Cancelar" fecha o modal por completo). Suíte completa: `npx vitest run` — 60 arquivos, 342 testes, todos passando (2 falhas de timeout em `SettingsPage.test.tsx`/`TransactionsPage.test.tsx` só ocorrem sob concorrência da suíte completa, confirmadas pré-existentes e não relacionadas — passam limpo quando rodadas isoladas); `tsc -b` sem erros; `oxlint` sem erros (só o mesmo warning `react(set-state-in-effect)` já presente em ~12 outras páginas do projeto, padrão aceito). | Captura Automatizada — Voz & Foto |
| FE-F3-04 | Rascunho de confirmação: S-CAP-03/S-CAP-05 (`DraftReviewBanner`, `AutoFillTag`) — tela mais crítica do produto para RNF-01 | Frontend | UX-FL-04, RNF-01/RNF-08, DIR-20 | Banner fixo não-descartável até ação explícita; nenhum timer/auto-confirmação/navegação automática (WCAG 2.2.1); tag "✨ sugerido" desaparece só ao editar o campo (RF-F3-01 AC3) | 3 dias | **Concluída — 2026-09-07.** Novo `DraftReviewBanner` (`frontend/src/components/domain/DraftReviewBanner.tsx`) + `AutoFillTag`/`AutoFillAttentionHint` (`AutoFillTag.tsx`), montados por `CaptureFab` (`CaptureFab.tsx` atualizado) como último passo dos dois canais de captura (voz `FE-F3-02`, foto `FE-F3-03`) — ponto de convergência do lote, conforme dependência já registrada na Seção 4.3. **Banner fixo/não-descartável**: `Modal` (`frontend/src/components/base/Modal.tsx`) ganhou a prop `dismissible` (default `true`, sem alterar nenhum chamador existente); `CaptureFab` monta `DraftReviewBanner` com `dismissible={false}` — Esc, clique no backdrop e o "✕" do cabeçalho ficam inertes, só os botões explícitos "Cancelar"/"Confirmar lançamento" saem da tela (testado em `Modal.test.tsx` e end-to-end em `CaptureFab.test.tsx`). **Fluxo de persistência real (`BE-F3-00`, sem mock)**: novo `frontend/src/lib/api/candidateTransactions.ts` (`createCandidateTransaction`/`deleteCandidateTransaction`/`confirmCandidateTransaction`) — ao montar, `DraftReviewBanner` já cria o `candidate_transaction` pendente (`raw_payload` = extração bruta); "Confirmar lançamento" chama a RPC `confirm_candidate_transaction` com os valores FINAIS (após qualquer edição do usuário, RF-F3-01 AC3) — único caminho que gera uma `Transaction` real; "Cancelar" exclui fisicamente o candidato ainda `pending` via `DELETE /candidate_transaction` (não `discard_candidate_transaction`, reservado para a lista de importação de `FE-F3-05` — `UX-SPEC.md`/migration de `BE-F3-00` documentam esse DELETE como "sem confirmação adicional" para o rascunho de voz/foto, por isso nenhum `ConfirmationDialog` aqui, diferente do padrão usado em exclusões reais de dado confirmado). Novo `frontend/src/lib/api/receiptOcr.ts` (`extractReceiptOcr`) chamado de verdade por `CaptureFab` ao confirmar a foto (mostra "Lendo o recibo..." enquanto aguarda); `ReceiptCameraCapture.tsx` (`FE-F3-03`) permanece **inalterado** — decisão de escopo documentada no próprio `CaptureFab.tsx` para não reabrir um componente já Concluído e testado. **AutoFillTag por campo**: cada campo com valor extraído (Data/Valor/Tipo/Categoria/Descrição) mostra "✨ sugerido" e perde a tag só quando aquele campo específico é editado (flag independente por campo, provado em `DraftReviewBanner.test.tsx` — editar Valor não afeta a tag de Data/Categoria/Descrição; só focar sem alterar não remove nada); campo que a extração tentou e não preencheu mostra `AutoFillAttentionHint` ("⚠ preencha", nunca bloqueia, RF-F3-02 AC3). Categoria: `category_suggestion_label` (rótulo textual) é casado contra a taxonomia real do usuário (match exato case-insensitive) — responsabilidade explicitamente delegada pelo contrato (`_shared/ocrProvider.ts`), sem match a UI fica em branco para seleção manual. **Falha de OCR nunca bloqueia (UX-SPEC S-CAP-04 literal)**: 502/503/erro de rede em `/receipt-ocr` não mostra tela de erro com retry — `CaptureFab` abre o rascunho direto com todos os campos em branco para preenchimento manual (diferente do tratamento de erro de `/voice-capture`, que já é escopo interno de `VoiceRecorderUI`/`FE-F3-02`, inalterado). Erro de rede em qualquer chamada desta tarefa (criação do candidato, confirmação, descarte) nunca perde o rascunho — mensagem inline + "Tentar novamente" (criação) ou permanece na tela com os valores editados intactos (confirmação/descarte), nunca reseta o formulário. **3 decisões de interpretação pequenas, documentadas no próprio `DraftReviewBanner.tsx`** (nenhuma exigiu pausar/`BLOCKERS.md` — detalhe de implementação, não lacuna real do UX-SPEC.md nem desvio de escopo/estimativa): (1) "Conta"/"Forma de pagamento" nunca mostram `AutoFillTag` — nenhum dos dois contratos publicados (`/voice-capture` v0.22.0, `/receipt-ocr` v0.21.0) sugere esses campos (nem a fala nem a imagem indicam qual conta/forma foi usada), mesmo o ASCII ilustrativo de `UX-SPEC.md` Seção 3.3 mostrando "✨sugerido" ao lado deles — sempre campos manuais aqui, e sempre exigidos explicitamente porque a RPC `confirm_candidate_transaction` recebe `p_account_id`/`p_payment_method_id` diretos, sem depender do trigger de resolução implícita usado pelo formulário manual unificado (`ADR-016`); (2) captura por foto não tem "Tipo" (entrada/saída) extraído por nenhum contrato — assume "Saída" sem tag, editável livremente; (3) descrição da foto usa `merchant_name` como valor sugerido inicial (rótulo mais próximo disponível), já que o contrato de OCR não tem um campo de "descrição" dedicado. **Testes novos**: `DraftReviewBanner.test.tsx` (17 casos — banner fixo, criação do candidato por fonte, AutoFillTag por campo incluindo o caso de editar 1 campo sem afetar os demais e o de só focar sem remover a tag, AutoFillAttentionHint, match de categoria, habilitação do Confirmar só com todos os campos obrigatórios, confirmação com sucesso, falha de confirmação preservando os valores editados, cancelamento com/sem candidato já criado, falha de descarte mantendo o banner aberto, falha de criação com retry, e o **teste crítico de RNF-01/WCAG 2.2.1 com fake timers avançando 10 minutos confirmando zero chamada automática de confirmação/descarte/callback** — PASS), `AutoFillTag.test.tsx` (2 casos), `Modal.test.tsx` (+3 casos para `dismissible={false}`), `CaptureFab.test.tsx` (+5 casos: fluxo de voz até o rascunho, fluxo de foto com OCR real e miniatura do recibo, falha de OCR abrindo rascunho em branco, confirmação end-to-end com toast de sucesso), `candidateTransactions.test.ts` (10 casos), `receiptOcr.test.ts` (2 casos) — todos PASS. Suíte completa: `npx vitest run` — 65 arquivos, 388 testes, todos passando (1 falha isolada de `UnlockPage.test.tsx` só sob concorrência da suíte completa, confirmada pré-existente e não relacionada — passa limpo quando rodada isolada, mesmo padrão já registrado em `FE-F3-03`); `npx tsc -b` sem erros; `npx oxlint` sem erros novos (mesmos 15 warnings pré-existentes `react(set-state-in-effect)`/`react(only-export-components)` já presentes antes desta tarefa). | Captura Automatizada — Voz & Foto |

**Nota de execução — Lote "Captura Automatizada — Voz & Foto" concluído (2026-09-07)**: as 6 tarefas do lote (`BE-F3-00`, `BE-F3-01`, `BE-F3-02`, `FE-F3-01`, `FE-F3-02`, `FE-F3-03`, `FE-F3-04`) estão todas `Concluída`, com `FE-F3-04` fechando o ponto de convergência de voz+foto conforme a dependência registrada na Seção 4.3 (`FE-F3-04 | FE-F3-02, FE-F3-03, BE-F3-00 (contrato) | Contrato | — (é o ponto de convergência de voz+foto)`). Pendências não-bloqueantes já registradas nas tarefas de origem, sem novidade introduzida aqui: `GOOGLE_VISION_API_KEY` ainda não configurada em nenhum ambiente (`BE-F3-01`) e nenhum smoke test empírico com a Web Speech API/Google Vision reais foi rodado (`BE-F3-01`/`BE-F3-02`) — ambas seguem como responsabilidade operacional do Backend, sem prazo fixo. Este lote ainda não foi submetido a `/validar` — `QA-F3-01`/`QA-F3-02` (Seção 4, Fechamento & Regressão Fase 3) continuam `Não iniciada` e são o próximo passo natural antes de qualquer promoção de status para "Validado"; essa decisão fica para o usuário/comando `/validar`, fora do escopo desta tarefa.

**Nota de validação — Lote "Captura Automatizada — Voz & Foto" (2026-09-08)**: `/validar` executado sobre as 7 tarefas acima. Veredito QA: **Aprovado** (`QA-REPORT.md` Seção 21) — todas as 7 tarefas aprovadas, nenhuma reprovação. Veredito DevSecOps: **Aprovado com débito** (`SECURITY-REVIEW.md` Seção 1.31) — achado `SEC-DEBT-015` (Média, não bloqueante, condição de bloqueio automático antes de `BE-F3-03`/`BE-F3-04`), agendado como `BE-DEBT-04` acima. Checagem estrutural: **Consistente com ressalva** — 7/7 tarefas `Concluída`, nenhuma dependência da Seção 4.3 órfã relativa a este lote, nenhuma tarefa `Bloqueada`. **Lote fecha como `Validado (com ressalvas)`.** `QA-F3-01`/`QA-F3-02` continuam corretamente `Não iniciada` — são tarefas de fechamento de toda a Fase 3 (Seção 4, "Fechamento & Regressão Fase 3"), não deste lote específico, e não bloqueiam este veredito.
| FE-F3-05 | Importação: S-CAP-06/07 (`CandidateList`, `ReconciliationHint`, seleção em lote) | Frontend | UX-FL-05, FL-05 | Itens sinalizados como possível duplicata vêm desmarcados por padrão (RF-F3-03 AC2); nada persiste antes da confirmação de seleção (AC3) | 2.5 dias | **Concluída — 2026-09-08.** `frontend/src/components/domain/StatementImportFlow.tsx` (S-CAP-06 upload + S-CAP-07 revisão/confirmação em lote, orquestrador novo) + `frontend/src/components/domain/CandidateList.tsx` (lista selecionável — contador "N transações encontradas · M selecionadas", "Selecionar todas"/"Limpar seleção", `⚠`) + `frontend/src/components/domain/ReconciliationHint.tsx` (link "ver lançamento existente" ao lado de duplicata), todos **[NOVO]** conforme `UX-SPEC.md` Seção 3. Ponto de entrada: `CaptureFab` ganhou a 4ª opção "Importar extrato" (mesmo padrão de Modal/BottomSheet de voz/foto). Client novo `frontend/src/lib/api/statementImport.ts` (`extractStatementImport`, chama `/statement-import` real via `invokeEdgeFunction`, `BE-F3-03` v0.23.0) + 2 funções novas em `frontend/src/lib/api/candidateTransactions.ts` (`createImportBatch` — `POST /import_batch` — e `discardCandidateTransaction` — `POST /rpc/discard_candidate_transaction`, hoje sem chamador real neste fluxo, ver nota abaixo) + tipos `ImportBatch`/`NewImportBatch` novos em `types.ts`. **AC literal (RF-F3-03 AC2)**: a seleção inicial de `StatementImportFlow` marca todo candidato com `duplicate_of_transaction_id !== null` como **desmarcado**, os demais como marcados — provado por teste explícito. **AC literal (AC3/RNF-01)**: teste dedicado ("nenhuma chamada de persistência... antes do clique em 'Confirmar'") chama `expect(createImportBatch/createCandidateTransaction/confirmCandidateTransaction).not.toHaveBeenCalled()` logo após a revisão carregar e os seletores do lote serem preenchidos — só o clique explícito em "Confirmar N lançamentos" dispara qualquer escrita. **Decisão de design central, documentada no próprio componente (gap real de `UX-SPEC.md`, não deste código)**: o schema real de `transactions` (`transactions_non_transfer_requires_method_and_category`) exige `payment_method_id`/`category_id` para todo lançamento não-transferência, mas o wireframe de S-CAP-07 não mostra nenhum campo de forma de pagamento/categoria por item da lista. Resolvido com **um único seletor de Forma de Pagamento + um único seletor de Categoria aplicados a toda a seleção do lote** (`account_id` já escolhido em S-CAP-06, reaproveitado para todos os candidatos) — mesmo precedente do `Bloqueio 008`/`FE-M-04` (preenchimento funcional mínimo, sinalizado ao UX/UI, não bloqueia a cadeia); nenhuma entrada nova em `BLOCKERS.md`, é detalhe de tela resolvível pelo Frontend. **Achado/decisão pequena adicional**: `ReconciliationHint` navega para `/lancamentos` (mesmo destino de "Lançamento manual" em `CaptureFab`, RN-20) — `router.tsx` não expõe rota de detalhe de lançamento único para um deep-link mais preciso; documentado no próprio componente como candidato a revisão de UX futura. **Fluxo de confirmação em lote** (`BE-F3-00`): 1 `POST /import_batch` (`source: "import"`, status no `DEFAULT` da coluna — nenhuma tarefa decomposta lê esse campo de volta, decisão pequena documentada em `candidateTransactions.ts`), depois, por candidato selecionado em ordem, `POST /candidate_transaction` seguido de `POST /rpc/confirm_candidate_transaction` com os valores finais (conta de S-CAP-06 + forma de pagamento/categoria do lote + `kind`/`amount_cents`/`transaction_date`/`description` do `raw_payload` do candidato). **Falha parcial (RNF-01)**: candidato que falha ao confirmar nunca reverte os já confirmados — sai da lista quem já foi persistido, quem falhou volta selecionado na tela para nova tentativa, com `Alert` claro citando quantos falharam; provado por teste dedicado. `discardCandidateTransaction` foi implementado (RPC já existe desde `BE-F3-00`) mas não tem chamador neste fluxo — candidato não-selecionado da resposta de `/statement-import` nunca chega a ser criado como linha `pending` (a Edge Function não persiste nada, `BE-F3-03`), então não há o que descartar; função documentada como pronta para uso futuro (ex. Open Finance, `FE-F3-06`). Testes: `StatementImportFlow.test.tsx` (7 casos — upload chamando `/statement-import` com o base64/formato/conta corretos, duplicata desmarcada por padrão vs. demais marcados, seleção individual/"Selecionar todas"/"Limpar seleção", botão "Confirmar" só habilita com seleção+forma de pagamento+categoria, nenhuma persistência antes de "Confirmar", ordem exata `import_batch`→`candidate_transaction`→`confirm` por item, falha parcial preservando os já confirmados) + `CandidateList.test.tsx` (3 casos — vazio, `ReconciliationHint` só em duplicata, contador) — todos PASS. Regressão completa: `npx vitest run` — **397/398 passando** (10 novos desta tarefa); a única falha é `UnlockPage.test.tsx` (MVP, já documentada como flaky sob concorrência de suíte completa desde `FE-F2-09`, sem qualquer alteração de código desta tarefa em `src/lib/auth`/`src/pages/auth` — reexecutada isolada, PASS). `npx tsc -b` limpo; `npm run build` gera bundle de produção válido; `npm run lint` sem novo warning/erro nos arquivos desta tarefa (só os `react(set-state-in-effect)` pré-existentes de outras páginas, convenção já aceita). `API-CONTRACT.yaml` conferido — `/import_batch`, `/candidate_transaction`, `/rpc/confirm_candidate_transaction`, `/rpc/discard_candidate_transaction` já publicados desde `BE-F3-00`, nenhuma nota adicional necessária, sem bump de versão (uso client-side de contrato já existente) | Captura Automatizada — Importação de Extrato |
...
| FE-F3-07 | Relatório de evolução patrimonial: S-REP-02 (`LineChart`, filtro por conta) | Frontend | UX-FL-18 | Filtro "Todas as contas" disponível além de contas individuais | 1.5 dia | **Concluída — 2026-09-15.** Novo `LineChart` (`frontend/src/components/domain/LineChart.tsx`), SVG próprio sem lib externa — mesmo padrão de `BarChart`/`DonutChart` (S-REP-01/S-DASH-01): polyline com 1 ponto por mês, nota "Dados disponíveis a partir de [mês]" quando `items.length < 6` (nunca fabrica mês com zero, mesmo tratamento de RF-F2-10 AC2/DIR-06) e toggle "Ver como tabela" (alternativa textual a gráfico, WCAG). Novo `frontend/src/pages/reports/NetWorthEvolutionReportPage.tsx`: `Select` "Conta" com opção "Todas as contas" (valor vazio → `p_account_id` omitido) + 1 opção por conta ativa (`listAccounts({ onlyActive: true })`, já existente), estados de loading (`Skeleton`)/erro (`Alert`) no mesmo padrão de `IncomeExpenseReportPage`. Client novo `getNetWorthEvolution(accountId?)` em `frontend/src/lib/api/reports.ts` (`POST /rpc/get_net_worth_evolution`, `BE-F3-06` v0.24.0 — contrato real já publicado e concluído, **sem mock**) + tipo `NetWorthEvolutionItem` novo em `types.ts`. Rota `/relatorios/evolucao-patrimonial` registrada em `router/router.tsx`; item de navegação "Evolução Patrimonial" adicionado ao grupo final (sem rótulo) de `layout/AppLayout.tsx`, ao lado de "Relatórios" — decisão de interpretação pequena (não documentada em `UX-SPEC.md`/RN-20 explicitamente, mas necessária para a tela ser alcançável; `UX-SPEC.md` Seção 2.2 menciona um card no Dashboard linkando para S-REP-02 em Fase 3, mas o precedente já estabelecido por `FE-F2-08`/S-REP-01 — também mencionado lá — não adicionou esse card, só o link de navegação lateral; este lote segue o mesmo precedente já aceito, sem reabrir `FE-F2-08`). Testes novos: `LineChart.test.tsx` (4 casos — janela parcial com nota textual, 6 meses sem nota, estado vazio, toggle de tabela) + `NetWorthEvolutionReportPage.test.tsx` (5 casos — visão consolidada por padrão chamando `get_net_worth_evolution(undefined)`, **AC literal**: select expõe "Todas as contas" + 1 opção por conta real, troca de conta chama a RPC com o `account_id` selecionado, estado de erro, estado de carregamento) — todos PASS. Regressão completa (executada em paralelo com `FE-F3-08`, rodando no mesmo lote): `npx vitest run` — 69 arquivos, 407 testes, todos passando; `npx tsc -b` sem erros. | Relatórios & Exportação (Fase 3) |
| FE-F3-08 | Exportação: S-REP-03 (seleção de período/formato, indicador de geração, download) | Frontend | UX-FL-19 | Usuário escolhe CSV ou PDF e recebe o arquivo correspondente ao período selecionado | 1.5 dia | **Concluída — 2026-09-15.** Novo `ExportReportPage` (`frontend/src/pages/reports/ExportReportPage.tsx`), rota `relatorios/exportar` (`router.tsx`) + item de navegação "Exportar Relatório" no grupo sem rótulo do desktop (`AppLayout.tsx`, mesmo precedente de `FE-F2-08`/S-REP-01 — sem hub "Mais" dedicado no mobile ainda, gap pré-existente não introduzido por esta tarefa). UX-SPEC S-REP-03 literal: `DatePicker` "De"/"Até" (default mês corrente via `currentMonthRange()`, `lib/date.ts`) + `fieldset`/radio nativo CSV/PDF (CSV pré-selecionado) + botão "Exportar" → indicador de geração (`role="status" aria-live="polite"`, "Gerando CSV/PDF...", `Button loading`) → card de sucesso com contagem de lançamentos e botão "Baixar arquivo". Novo client `frontend/src/lib/api/reportExport.ts` (`exportReport`/`downloadExportedFile`) consumindo `/report-export` **real** (`BE-F3-07`, já Concluída — contrato publicado em `API-CONTRACT.yaml` v0.25.0, nenhuma pendência de mock, por isso fecha `Concluída` e não `Em andamento`) via `invokeEdgeFunction`, mesmo padrão de `voiceCapture.ts`/`statementImport.ts`. **Decisão pequena documentada no próprio `reportExport.ts`**: a signed URL de `BE-F3-07` não define `Content-Disposition: attachment`, então `downloadExportedFile` busca o arquivo (`fetch` + `Blob`) e dispara o download via `<a download>` temporário, garantindo nome de arquivo/força de download em qualquer navegador em vez de um link direto cross-origin não confiável. **PWA/offline (diretriz Seção 1 do TASK.md)**: a tela escuta `online`/`offline` (mesmo princípio de `useOfflineQueue`) e desabilita "Exportar" com `Alert` explícito quando offline — exportação sempre exige rede (não há geração local), nunca falha silenciosa nem trava em estado de geração eterno. Alterar período/formato depois de já ter gerado um arquivo descarta o resultado anterior (evita baixar um arquivo desatualizado). **Testes automatizados** (TDD): `reportExport.test.ts` (7 casos — `exportReport` chamando `/report-export` com `start_date`/`end_date`/`format` corretos para CSV e para PDF, propagação de erro sem interceptar, `downloadExportedFile` buscando a signed URL/criando object URL/disparando `<a download>`/revogando a URL, e erro claro quando o fetch da signed URL falha) + `ExportReportPage.test.tsx` (8 casos — radio CSV pré-selecionado, **exportação CSV completa** com indicador de geração e botão de download final, **exportação PDF** via seleção do radio confirmando `format: "pdf"` no client e download subsequente, **conteúdo corresponde ao período selecionado** via datas exatas escolhidas pelo usuário passadas ao client, estado de erro com `Alert`, offline desabilitando "Exportar", período inválido — data final antes da inicial — desabilitando o botão com erro de campo) — todos PASS, cobrindo o critério de aceite literal (CSV e PDF, período correspondente). Regressão completa: `npx vitest run` — 419/420 passando (12 novos desta tarefa); a única falha é `UnlockPage.test.tsx`, já documentada como flaky sob concorrência de suíte completa desde `FE-F2-09`/reconfirmada isolada (PASS), sem qualquer alteração de código desta tarefa em `src/lib/auth`/`src/pages/auth`. `npx tsc -b` limpo; `npm run lint` sem warning/erro novo nos arquivos desta tarefa (mesmos warnings pré-existentes `react(set-state-in-effect)`/`react(only-export-components)` de outras páginas). `API-CONTRACT.yaml` conferido — `/report-export` já publicado desde `BE-F3-07` (v0.25.0), nenhuma nota adicional necessária, sem bump de versão (uso client-side de contrato já existente). | Relatórios & Exportação (Fase 3) |

**Nota de validação — Lote "Relatórios & Exportação (Fase 3)" (2026-09-15)**:
`/validar` executado sobre as 4 tarefas acima (`BE-F3-06`, `BE-F3-07`,
`FE-F3-07`, `FE-F3-08`). Veredito QA: **Aprovado** (`QA-REPORT.md` Seção 23)
— todas as 4 tarefas aprovadas, nenhuma reprovação crítica nem simples;
execução própria e independente da suíte completa do Frontend (`npx vitest
run` — 72 arquivos, 425/425 passando; `npx tsc -b` limpo) e leitura linha a
linha das 2 migrations + dos 2 testes SQL + dos testes Deno (não
reexecutados por ausência de `deno`/`supabase` CLI no ambiente de validação,
mesma limitação já registrada nas rodadas anteriores, corroborados por
leitura estrutural completa). Veredito DevSecOps: **Aprovado, sem débito de
segurança novo** (`SECURITY-REVIEW.md` Seção 1.33) — confirmado por leitura
direta que `get_net_worth_evolution`/`get_report_export_rows` são
`SECURITY INVOKER` (sem `security definer`) escopadas por `auth.uid()`, que
o bucket `exports` é privado com isolamento por pasta `<user_id>/...` e
signed URL de 300s (nunca URL pública), e que o Frontend novo não vaza dado
de outro usuário nem permite download sem sessão válida; nenhum `SEC-DEBT`
novo aberto. Checagem estrutural: **Consistente** — 4/4 tarefas `Concluída`,
nenhuma dependência da Seção 4.3 órfã/inconsistente relativa a este lote
(`BE-F3-06 | BE-M-07`, `BE-F3-07 | BE-M-06`, `FE-F3-07 | BE-F3-06 (contrato)`,
`FE-F3-08 | BE-F3-07 (contrato)` — todas satisfeitas, sem uso de mock),
nenhuma tarefa `Bloqueada`. **Pendência operacional, não achado de código**:
não há evidência, no ambiente de validação (sem CLI), de que a Edge Function
`report-export` tenha sido publicada no projeto Supabase real desde sua
conclusão (`DEPLOY.md` sem rodada de `/deploy` após 2026-09-09) — mesma
natureza do `BLOCKERS.md` Bloqueio 025 (deploy de Edge Function pendente por
bloqueio do classificador de permissão do ambiente do Executor); sinalizada
ao chapéu DevOps (`SECURITY-REVIEW.md` Seção 4) para resolução antes/durante
o próximo `/deploy`; **não** gerou tarefa nova em `Refatoração Lote-X` (não é
correção de código, é etapa de publicação, mesma decisão já tomada para o
Bloqueio 025 análogo, e para a pendência de migrations do lote "Retenção &
Descarte de Dado / Exclusão de Conta"). **Lote fecha como `Validado (com
ressalvas)`** — ressalva exclusivamente operacional (confirmação de
publicação da Edge Function pendente), sem nenhuma reprovação funcional ou
de segurança.

| FE-F3-09 | **[Nova — ADR-011]** Fluxo de exclusão de conta: confirmação em duas etapas com aviso textual da cauda residual de até 30 dias em backup já emitido | Frontend | ADR-011 ("Condição de revisão": fluxo delegado ao UX/UI, ainda **não especificado** em `UX-SPEC.md`) | Usuário só confirma exclusão após uma segunda confirmação explícita (nunca um único toque); tela exibe textualmente "dado pode persistir por até 30 dias em backup já emitido" antes da confirmação final; ao confirmar, chama `BE-F3-09` e encerra a sessão ao concluir | 1 dia (**preliminar** — usa `ConfirmationDialog`/`Modal` já definidos em FE-M-01 como base; sujeita a reestimativa quando UX-SPEC.md formalizar a tela definitiva, ver Seção 6.1) | **Concluída — 2026-09-15.** Implementada em `SettingsPage.tsx` (mesmo local das demais ações de conta — "Sair"/"Alterar PIN" — sem rota dedicada; decisão pequena, precedente já estabelecido pela própria tela), reaproveitando `ConfirmationDialog`/`Modal` de `FE-M-01` como base **exatamente conforme autorizado pela nota preliminar desta linha** (UX-SPEC.md ainda não formaliza esta tela — pendência conhecida, não-bloqueante, TASK.md Seção 6.1.1 item UX-01, mantida aqui por rastreabilidade mesmo já implementada). Fluxo em duas etapas explícitas distintas (nunca um único toque): etapa 1 (`ConfirmationDialog` "Excluir conta", botão "Continuar", `confirmVariant="primary"`) é só um aviso genérico de irreversibilidade que avança para a etapa 2, sem disparar nada; etapa 2 (`ConfirmationDialog` "Confirmar exclusão definitiva", botão destrutivo "Excluir permanentemente") mostra o texto literal "o dado pode persistir por até 30 dias em backup já emitido, até a rotação natural daquele snapshot (ADR-011)" antes de qualquer chamada de rede — só o clique neste segundo botão invoca `deleteAccount()`. Client novo `frontend/src/lib/api/deleteAccount.ts` (`deleteAccount`) chama `/delete-account` **real** (`BE-F3-09`, já Concluída, contrato publicado em `API-CONTRACT.yaml` v0.28.0, sem mock) via `invokeEdgeFunction` — mesmo padrão de `reportExport.ts`/`voiceCapture.ts`/`receiptOcr.ts` (JWT de sessão anexado automaticamente pelo client Supabase). Sessão só é encerrada (`signOut()` de `AuthContext`) após sucesso confirmado da Edge Function; erro (rede ou 4xx/5xx do contrato) mantém a sessão ativa e exibe `Alert` com a mensagem, sem fechar o modal automaticamente. **Testes automatizados** (TDD): `deleteAccount.test.ts` (2 casos — chama `delete-account` com corpo vazio e retorna o resultado, propaga falha sem interceptar) + 4 casos novos em `SettingsPage.test.tsx` provando literalmente os 4 ACs pedidos: (a) um único clique em "Excluir conta" não chama `deleteAccount` (só abre a etapa 1); (b) o aviso de retenção de até 30 dias aparece na etapa 2 antes de qualquer chamada, e só o clique em "Excluir permanentemente" dispara `deleteAccount`; (c) confirmação completa (duas etapas) chama a Edge Function e encerra a sessão (`signOut`) em caso de sucesso; (d) erro da Edge Function não encerra a sessão (`signOut` não chamado) e exibe a mensagem de erro em `role="alert"` — todos PASS. Regressão: `npx vitest run` — 424/425 passando (6 novos desta tarefa: 2 de `deleteAccount.test.ts` + 4 novos de `SettingsPage.test.tsx`); a única falha é `UnlockPage.test.tsx` (MVP, já documentada como flaky sob concorrência de suíte completa desde `FE-F2-09`, sem qualquer alteração de código desta tarefa em `src/lib/auth`/`src/pages/auth`). `npx tsc --noEmit` limpo; `npm run lint` sem warning/erro novo nos arquivos desta tarefa (mesmos warnings pré-existentes `react(set-state-in-effect)` de outras páginas). `API-CONTRACT.yaml` conferido — `/delete-account` já publicado desde `BE-F3-09` (v0.28.0), nenhuma nota adicional necessária, sem bump de versão (uso client-side de contrato já existente). | Retenção & Descarte de Dado / Exclusão de Conta |
...
| QA-F3-02 | Teste de acessibilidade WCAG 2.1 AA nos componentes novos sem equivalente de mercado: `VoiceRecorderUI`, `ReceiptCameraCapture`, `DraftReviewBanner`, `AutoFillTag`, `CandidateList` | QA | UX-SPEC Seção 3.3, Seção 5 | Nenhum achado crítico de acessibilidade aberto nos 5 componentes marcados **[NOVO]** | 1.5 dia | **Concluída — 2026-09-15.** Auditoria + correção nos 5 componentes `[NOVO]` listados (`frontend/src/components/domain/{VoiceRecorderUI,ReceiptCameraCapture,DraftReviewBanner,AutoFillTag,CandidateList}.tsx`). Ferramenta: `jest-axe` (`axe-core` 4.13) instalado como devDependency e registrado globalmente (`expect.extend(toHaveNoViolations)` em `frontend/src/test/setup.ts`), rodado contra o DOM renderizado (`@testing-library/react`) em todos os estados relevantes de cada componente (não só o estado inicial) — `VoiceRecorderUI`: listening/erro (permissão negada)/não suportado; `ReceiptCameraCapture`: câmera indisponível/streaming/pré-visualização; `DraftReviewBanner`: carregado com `AutoFillTag`/com `AutoFillAttentionHint` (foto sem campos extraídos)/com erro de criação+referência; `AutoFillTag`/`AutoFillAttentionHint`: isolados. **1 achado crítico encontrado e corrigido**: `AutoFillAttentionHint` ("⚠ preencha", `AutoFillTag.tsx`) usava o token compartilhado `text-warning` (`--color-warning: #b9862f`), que dá ~3,2:1 de contraste contra fundo branco e ~2,7:1 contra `bg-warning-soft` — abaixo do 4,5:1 exigido pelo WCAG 2.1 AA 1.4.3 (Contraste Mínimo) para texto pequeno (`text-xs`, não qualifica como "large text"); axe-core não sinaliza esse caso automaticamente porque `color-contrast` fica "inapplicable" em ambiente jsdom (sem renderização visual real), então a checagem foi feita manualmente calculando a luminância relativa (fórmula WCAG) dos hexadecimais reais do design system (`frontend/src/index.css`). Corrigido trocando só a cor deste componente (`text-[#946b26]`, ~4,57:1 contra branco e ~4,3→4,5:1 contra `neutral-50`) — **sem** alterar o token `--color-warning` compartilhado, que outros componentes fora do escopo desta tarefa (`Badge`, `ProgressBar`, `OfflineSyncBadge`) ainda consomem sobre fundos diferentes; mudança de token global ficaria fora da autoridade desta tarefa (afetaria componentes não auditados aqui) e é sinalizável ao Coordenador como melhoria de padrão futura, não um achado bloqueante desta tarefa. Demais achados: nenhum — `role="alert"`/`aria-live="polite"` já usados corretamente nos 3 componentes com estado assíncrono (`VoiceRecorderUI`/`ReceiptCameraCapture`/`DraftReviewBanner`), ícones puramente decorativos (`Mic`, `X`, `Camera`, `Upload`, `⚠` de duplicata em `CandidateList`, `<video>` do viewfinder) corretamente `aria-hidden`, todos os controles interativos são `<button>`/`<input>`/`<label htmlFor>` nativos (nunca `<div onClick>`, sempre alcançáveis por teclado), grupo "Tipo de lançamento" com `role="group"`+`aria-label`, input de arquivo `sr-only` com nome acessível via `<label htmlFor>`, checkbox de `CandidateList` com nome acessível via `<label>` envolvente (0 "input mudo"). **Testes automatizados** (TDD, adicionados aos arquivos de teste já existentes de cada componente — nenhum arquivo novo): `AutoFillTag.test.tsx` (+3 casos, incl. o achado de contraste corrigido), `CandidateList.test.tsx` (+4 casos), `VoiceRecorderUI.test.tsx` (+5 casos), `ReceiptCameraCapture.test.tsx` (+5 casos), `DraftReviewBanner.test.tsx` (+6 casos) — 23 casos novos de acessibilidade, todos usando `axe(container)` + `toHaveNoViolations()` mais asserções pontuais (nome acessível, `aria-hidden`, `role`, tag nativa). **Resultado**: `npx vitest run` (suíte completa) — **448/448 passando, 72/72 arquivos**, nenhuma regressão introduzida (baseline antes desta tarefa não incluía os 23 casos novos); `npx tsc -b` limpo; `npm run lint` sem warning/erro novo nos arquivos desta tarefa (mesmos warnings pré-existentes `react(set-state-in-effect)` de outras páginas, incluindo 1 em `ReceiptCameraCapture.tsx` já presente antes desta tarefa). Escopo desta tarefa cobre só os 5 componentes `[NOVO]` listados no critério de aceite — não uma auditoria de acessibilidade de todo o app (achados potenciais em `Badge`/`ProgressBar`/`OfflineSyncBadge`, que também usam `text-warning`, ficam fora do escopo literal desta linha, sinalizados acima só para rastreabilidade). | Fechamento & Regressão Fase 3 |
...
| QA-F3-04 | **[Nova — ADR-011]** Casos de teste para os jobs de expurgo (`BE-F3-08`: candidato/foto 30 dias, foto de confirmado 90 dias, export 24h; `BE-F3-10`: rotação de backup) e para o fluxo de exclusão de conta (`BE-F3-09`/`FE-F3-09`) | QA | ADR-011 | Teste automatizado confirma que dado além do prazo é removido e que dado dentro do prazo **não** é removido (fronteira testada nos dois sentidos, para cada categoria); teste de exclusão de conta confirma ausência de qualquer linha remanescente em `public`, objeto no Storage e usuário em Supabase Auth associados ao `user_id` excluído | 1 dia | **Concluída — 2026-09-15. Auditoria de cobertura (mesmo padrão de `QA-M-01`/`QA-REF-02..05`), não reimplementação** — toda a cobertura exigida pelo critério de aceite já existia, escrita sob TDD pelas próprias tarefas de implementação; nenhum caso redundante foi inventado, e o único caso concretamente faltante foi escrito nesta tarefa. Mapeamento literal, critério por critério: (1) **Candidato/foto 30 dias** (`BE-F3-08`) — `supabase/tests/be_f3_08_data_retention_purge_jobs.test.sql` CASO 3/4/5 cobre os 3 subcasos da regra (descartado, pending com `ImportBatch`, pending avulso) nos 2 sentidos da fronteira (31 dias removido, 5 dias mantido) + CASO 6 confirma que candidato confirmado (400 dias) nunca é removido (retenção indefinida) — 7/7 casos, execução real via `supabase db query --linked --file` já documentada PASS na própria nota de status de `BE-F3-08`. "Foto" desta linha: **sem objeto** — `ADR-020`/Bloqueio 024 (`BLOCKERS.md`, Resolvido) determinou que foto de recibo nunca é persistida (decisão definitiva do stakeholder), então não há objeto de Storage a expurgar nesta categoria; o 3º sub-job (`confirmed_receipt_photo_purge`) foi removido do código por completo, não existe mais nem como skip — nada a testar aqui além do que já é confirmado pela ausência do sub-job. (2) **Foto de confirmado 90 dias** (`BE-F3-08`) — mesmo tratamento: **sem objeto**, mesma `ADR-020`, nenhuma lacuna real (não é a mesma coisa que "não testado"; é "não existe mais para testar"). (3) **Export 24h** (`BE-F3-08`) — mesmo arquivo `.sql`, CASO 7, fronteira nos 2 sentidos (25h sinalizado como expirado, 1h mantido) + isolamento por bucket (objeto de outro bucket com a mesma idade "expirada" nunca é sinalizado) — 100% do critério coberto. (4) **Rotação de backup** (`BE-F3-10`) — `supabase/functions/backup-export/lib.test.ts` já continha, desde `BE-M-10`, os 2 casos exatos ("objectsToDelete mantém só os 30 mais novos, retorna o excedente" e "com <= 30 chaves não apaga nada"), confirmados nesta tarefa por leitura direta do arquivo (linhas 94-104) — cobre o máximo de 30 snapshots nos 2 sentidos da fronteira, sem necessidade de caso novo. (5) **Exclusão de conta — ausência de linha em `public`** (`BE-F3-09`) — `supabase/tests/be_f3_09_delete_account_data.test.sql`, 5 casos com dado REAL nas 19 tabelas "ownable" + `profiles` (todas as tabelas hoje existentes, auditadas linha a linha no cabeçalho da migration) para um usuário-alvo e um usuário de controle: CASO 2 confirma 0 linhas restantes do alvo somando as 20 tabelas, CASO 3 confirma as 20 linhas do usuário de controle intactas (isolamento), CASO 4 confirma idempotência, CASO 5 confirma rejeição de `p_user_id NULL` — critério coberto de forma literal e concreta (não é teste de lógica pura, é `DELETE` real revertido por `ROLLBACK`); mesma pendência operacional já registrada por `BE-F3-08`/`BE-F3-09` (execução via `supabase db query --linked --file` não fica disponível neste ambiente do Executor, sem CLI/credencial do projeto linkado) — SQL pronto para quem tiver a CLI/credencial rodar, não uma lacuna de especificação/cobertura. (6) **Exclusão de conta — objeto no Storage e usuário em Supabase Auth** (`BE-F3-09`) — confirmado que **não há, em nenhuma Edge Function deste projeto (auditado em `supabase/functions/*/lib.test.ts` e `*/index.ts`), o padrão de teste com client Supabase mockado para orquestração real contra Storage/Auth/S3** — toda Edge Function deste projeto (`backup-export` contra S3 real, `delete-account` contra Storage/Auth reais, etc.) segue uniformemente o mesmo padrão de 2 camadas: (i) `lib.ts` cobre com teste automatizado toda a lógica pura que decide O QUE remover — para `delete-account`, `buildExportObjectPaths`/`chunk`/`totalDeletedRows` (`supabase/functions/delete-account/lib.test.ts`, 11/11 PASS) — e (ii) a chamada real à Storage API (`adminClient.storage.from('exports').remove(...)`, `index.ts` linha 277) e à Admin API do GoTrue (`adminClient.auth.admin.deleteUser(user.id)`, `index.ts` linha 295) é verificada por smoke test real no deploy, nunca por integração local mockada — mesmo padrão já aceito para a rotação de backup contra S3 real (`BE-F3-10`) e para todo outro Edge Function do projeto; não é uma lacuna desta tarefa, é a convenção de teste já estabelecida e consistente em todo o projeto, formalizada aqui por auditoria em vez de reinventada. Refatorar `index.ts` para permitir client injetável só para viabilizar teste mockado seria uma mudança de arquitetura de Backend fora do escopo de uma tarefa de QA de confirmação — sinalizável ao Coordenador como melhoria futura de padrão de teste (não bloqueante, não um achado desta auditoria). (7) **Frontend** (`FE-F3-09`) — `frontend/src/pages/settings/SettingsPage.test.tsx` (10/10 PASS, revalidado nesta sessão via `npx vitest run`) cobre os 4 ACs literais do fluxo de duas etapas (clique único não dispara exclusão; aviso de "até 30 dias em backup" aparece na etapa 2 antes de qualquer chamada; confirmação completa chama `deleteAccount()` e encerra sessão; erro mantém sessão ativa) + `frontend/src/lib/api/deleteAccount.test.ts` (2/2, chamada real ao endpoint sem mock). **Nenhum teste novo foi necessário** — toda a cobertura pedida pelo critério de aceite já existia (mesmo padrão de confirmação, não invenção, já usado por `BE-F3-10` dentro desta mesma resolução do ADR-011). | Retenção & Descarte de Dado / Exclusão de Conta |

**Nota de validação — Lote "Retenção & Descarte de Dado / Exclusão de Conta"
(2026-09-15)**: `/validar` executado sobre as 5 tarefas acima (`BE-F3-08`,
`BE-F3-09`, `BE-F3-10`, `FE-F3-09`, `QA-F3-04`). Veredito QA: **Aprovado**
(`QA-REPORT.md` Seção 22) — todas as 5 tarefas aprovadas, nenhuma reprovação
crítica nem simples; verificação direta de código (não da nota do Executor)
no ponto de maior consequência do lote — `validateTargetUserId`/`GRANT`/
`REVOKE` de `public.delete_user_data`. Veredito DevSecOps: **Aprovado, sem
débito de segurança novo** (`SECURITY-REVIEW.md` Seção 1.32) — leitura linha
a linha da migration `SECURITY DEFINER` e da Edge Function `delete-account`
não encontrou achado de severidade alta/crítica nem de baixa/média; nenhum
`SEC-DEBT` novo aberto. Checagem estrutural: **Consistente** — 5/5 tarefas
`Concluída`, nenhuma dependência da Seção 4.3 órfã/inconsistente relativa a
este lote (a lacuna de `BE-F3-09` não cobrir a futura tabela de Open Finance,
porque `BE-F3-04` ainda não existe, está documentada explicitamente no
cabeçalho da própria migration — mesmo padrão já aceito para `BE-F3-08`/foto
de recibo, não uma inconsistência nova), nenhuma tarefa `Bloqueada`.
**Pendência operacional, não achado de código**: as migrations
`20260915090000_be_f3_09_delete_account_data.sql` e
`20260909122000_be_f3_08_adr020_drop_photo_comment.sql` ainda não foram
aplicadas ao projeto Supabase real vinculado (mesma natureza do
`BLOCKERS.md` Bloqueio 025, deploy de Edge Function pendente por bloqueio do
classificador de permissão do ambiente do Executor) — sinalizada ao chapéu
DevOps (`SECURITY-REVIEW.md` Seção 4) para resolução antes/durante o próximo
`/deploy`; **não** gerou tarefa nova em `Refatoração Lote-X` (não é correção
de código, é etapa de publicação, mesma decisão já tomada para o Bloqueio
025 análogo). **Lote fecha como `Validado (com ressalvas)`** — ressalva
exclusivamente operacional (aplicação de migrations pendente), sem nenhuma
reprovação funcional ou de segurança.
...
| FE-RS-05 | Redesign visual de `S-DASH-01` (desktop `Main.dc.html` + mobile `DashboardMobile.dc.html`): reorganizar `DashboardPage.tsx` — card de saldo hero (`--r-lg`/`--shadow-md`), 3 KPIs `grid-cols-3`, linha 2 colunas (donut+legenda \| orçamentos), "Últimos lançamentos" largura total abaixo — usando exclusivamente tokens/componentes do Lote 0, sem alterar dado/cálculo/navegação (RF-RS-01 AC1-4/RN-20) | Frontend | `UX-SPEC.md` Seção 2.2 ("`S-DASH-01` — Redesign visual v2.0"); RF-RS-01 AC1-6 | Composição segue o mapeamento linha a linha da `UX-SPEC.md` (ordem de blocos, grid 2 colunas a partir de `lg`); nenhum dado/cálculo/comportamento de navegação (toque na fatia do donut → lista filtrada) é alterado; mobile preserva single-column (RNF-10); 0 chamada de API nova | 2.0 dias | **Concluída (2026-09-15, corrigida em fix-loop após achado de spec-compliance).** `DashboardPage.tsx` reorganizado em **2 grids de 2 colunas a partir de `lg`**, batendo com o wireframe literal da `UX-SPEC.md` Seção 2.2 ("`S-DASH-01` revisado para desktop", linhas 650-684): **Linha 1** (`grid-cols-1 lg:grid-cols-5`) — coluna esquerda `lg:col-span-3` (card de saldo hero + 3 KPIs), coluna direita `lg:col-span-2` (somente donut+legenda, sem "Orçamentos" empilhado). **Linha 2**, logo abaixo (`grid-cols-1 lg:grid-cols-2`) — "Orçamentos do mês" e "Últimos lançamentos" lado a lado, ambos ocupando a largura toda dividida em 2 — este é o 2º ganho de altura de rolagem exigido por RF-REF-01 AC3, que a versão anterior (Orçamentos empilhado sob o donut + Últimos lançamentos solto em largura total) não produzia. Correção feita após revisão inline apontar que a implementação original divergia do mapeamento linha a linha explícito da UX-SPEC (wireframe ASCII + texto da Seção 2.2), não uma reinterpretação válida. Ordem no DOM preservada de propósito (saldo → 3 KPIs → donut → orçamentos → últimos lançamentos) — idêntica à sequência mobile do wireframe base/`DashboardMobile.dc.html` (Seção 2.2, "permanece exatamente como está... é o que se aplica abaixo do breakpoint desktop"), então abaixo de `lg` ambos os grids colapsam para 1 coluna, nessa ordem, sem precisar de `order`/JS (RNF-10 intocado). `Card` (`components/base/Card.tsx`, Lote 0) mantém a prop `elevation?: "sm" | "md"` (classe estática via ternário) usada só pelo card de saldo; nenhum novo componente criado. Badge "sincronizado agora" em `tone="primary"`. 3 KPIs em `grid-cols-3` incondicional. Nenhuma chamada de API nova, nenhuma mudança em `load()`/cálculo de saldo-KPIs-donut-orçamentos, nenhuma mudança na navegação do clique na fatia do donut (`onSliceClick` intocado) nem no botão "ver todos". Migração para `<Num />` **não** feita aqui (escopo de `FE-RS-06`) — `formatCentsToBRL()` mantido. Suíte completa reexecutada após a correção: **448/448 `PASS`** (72 arquivos), 0 regressão. | Dashboard (Redesign v2.0) |
| FE-RS-06 | Migrar para `<Num />` os pontos de chamada de `formatCentsToBRL()` já existentes em `DashboardPage.tsx` (saldo consolidado; 2 KPIs monetários, separando a seta ↑/↓ do valor em nós distintos; "Lançamentos" como `format="count"`) e em `DonutChart.tsx:93` (separar valor de moeda e percentual, hoje concatenados) — `ADR-019`, `DIR-41` | Frontend | `UX-SPEC.md` Seção 2.2 ("Migração `Num` neste lote"); `ADR-019` | Todo número do Dashboard renderiza via `<Num />`, nunca `{formatCentsToBRL(x)}` cru; seta e valor são nós DOM distintos; valor de moeda e percentual do donut são dois `<Num />` distintos, não uma `string` concatenada; nenhuma regressão de teste existente do Dashboard | 0.5 dia | **Concluída (2026-09-16).** `DashboardPage.tsx`: saldo consolidado, os 2 KPIs monetários ("Entradas do mês"/"Saídas do mês") e "Lançamentos" migrados para `<Num format="currency"/>`/`<Num format="count"/>`. Seta (↑/↓) extraída para `<span aria-hidden="true">` irmão do `<Num>`, nunca mais concatenada na mesma string/nó de texto — glifo puramente decorativo (redundante com a cor `text-income`/`text-expense` e o rótulo textual do card), marcado `aria-hidden` para não duplicar informação lida por leitor de tela. Migrado também, por decorrência direta do critério de aceite ("todo número do Dashboard", não presumido limitado à lista literal da tarefa), o valor de cada item de "Últimos lançamentos" (mesmo padrão seta+`<Num>` como nós distintos) — `formatCentsToBRL()` sem nenhum ponto de chamada cru restante no arquivo (grep confirma). `DonutChart.tsx:93`: legenda migrada para dois `<Num>` distintos (`format="currency"` + `format="percent"`), a `string` `` `${formatCentsToBRL(...)} (${pct}%)` `` eliminada; a mesma duplicação existia também na visão-tabela alternativa (`showTable`, WCAG Seção 5) e foi migrada junto pelo mesmo motivo — nenhum `formatCentsToBRL()` cru restante em `DonutChart.tsx` (import removido). `aria-label` do `<svg role="img">` (resumo textual do gráfico) mantido como string simples, fora do escopo (é atributo de acessibilidade, não nó de texto renderizado). Nenhuma lógica de dado/cálculo/navegação alterada (`onSliceClick`, `load()`, agregação de `incomeCents`/`expenseCents` intocados). 2 testes de `DashboardPage.test.tsx` ajustados (matcher de texto exato "42/0 este mês" trocado por matcher de função por já não ser um único nó de texto — mesma asserção semântica, ciente da fragmentação de nós introduzida pela migração); nenhuma asserção de comportamento alterada. Suíte completa reexecutada: **448/448 `PASS`** (72 arquivos), `npm run build` sem erro, `oxlint` sem warning novo (os 16 pré-existentes, nenhum em `DashboardPage.tsx`/`DonutChart.tsx` além do `set-state-in-effect` já presente antes desta tarefa, linha intocada). | Dashboard (Redesign v2.0) |
| FE-RS-15 | **[Novo — 2026-09-04, `UX-03` Achado 2]** Definir a família de tokens `--color-chart-1` a `--color-chart-8` em `frontend/src/index.css` (bloco `@theme`), derivada da paleta v2.0 (`FE-RS-01`), mantendo contraste ≥3:1 sobre `--surface`/`--bg` (elemento gráfico não-textual, `UX-SPEC.md` Seção 5); migrar `components/domain/DonutChart.tsx:16` (`PALETTE`, hoje 8 hex hardcoded) para consumir esses 8 tokens em vez de valor literal — `DIR-47` | Frontend | `UX-SPEC.md` Seção 3.0.1, Achado 2 (`design-system-consistency-check`, `UX-03`); RF-MVP-06 | `index.css` declara os 8 tokens `--color-chart-N`; `DonutChart.tsx` não contém mais nenhum hex literal (grep confirma); cada tom mantém contraste ≥3:1 contra o fundo do gráfico (cálculo registrado, não estimado); nenhuma mudança de dado/comportamento do donut (RF-MVP-06, toque na fatia → lista filtrada preservado); nenhuma regressão nos testes existentes de `DonutChart`/Dashboard | 0.5 dia | **Concluída (2026-09-16).** `frontend/src/index.css`: declarados os 8 tokens `--color-chart-1` a `--color-chart-8` no bloco `@theme`, na mesma família visual "verde/terracota sobre creme" da paleta v2.0 — `chart-1`/`chart-2`/`chart-3`/`chart-4` reusam literalmente o valor de `--color-primary`/`--color-expense`/`--color-warning`/`--color-danger` (mesma lógica de nomes distintos com mesmo valor já usada por `--color-income`); `chart-5` a `chart-8` são 4 matizes adicionais (petróleo `#1f5f74`, ameixa `#6b4a8f`, verde-azulado `#2f7d6e`, ardósia `#55606e`) necessários porque só 4 tons semânticos não bastam para 8 categorias simultâneas sem repetir cor. Contraste WCAG 1.4.11 (não-textual, limiar 3:1) calculado de forma independente (luminância relativa sRGB, script Node dedicado — mesmo método de `QA-BUG-001`), contra `--color-surface` (#ffffff) e `--color-surface-alt`/`--bg` (#faf8f3): chart-1 `#2f6b4f` 6.29:1/5.93:1; chart-2 `#b4483a` 5.35:1/5.04:1; chart-3 `#b9862f` 3.22:1/3.04:1; chart-4 `#752f26` 9.58:1/9.03:1; chart-5 `#1f5f74` 7.13:1/6.71:1; chart-6 `#6b4a8f` 7.00:1/6.59:1; chart-7 `#2f7d6e` 4.91:1/4.62:1; chart-8 `#55606e` 6.39:1/6.02:1 — todos os 8 tons acima do limiar de 3:1 nos dois fundos (chart-3 é o mais próximo do piso, 3.04:1 contra `--bg`, ainda assim aprovado). `DonutChart.tsx`: `PALETTE` (linha 16) migrado dos 8 hex literais para os 8 `var(--color-chart-N)` correspondentes, na mesma ordem (grep confirma zero hex literal restante no arquivo); nenhuma mudança em `onSliceClick`, cálculo de `pct`/`dash`/`offsets`, ou na migração `<Num />` já feita por `FE-RS-06` (preservada intocada, conforme instrução explícita de não reverter). Suíte completa reexecutada: **448/448 `PASS`** (72 arquivos, incluindo `DonutChart.test.tsx` 3/3 e `DashboardPage.test.tsx` 4/4), nenhuma regressão. | Dashboard (Redesign v2.0) |
| FE-RS-07 | Redesign visual de `S-TXN-01`/`S-TXN-02` (desktop `Lancamentos.dc.html` + mobile `LancamentosMobile.dc.html`): cabeçalho, filtros em chips, card de resumo do período, lista agrupada por dia com a hierarquia de RN-18 preservada (subcategoria linha 1 destaque `--text` semibold; descrição·forma de pagamento linha 2 em `--text-2`, nunca `--text-3`); `ShortcutBar`/`ShortcutChip` (RF-REF-03) obrigatório e composto visualmente na linguagem v2.0 mesmo ausente do mockup (AMB-19) | Frontend | `UX-SPEC.md` Seção 2.2 ("`S-TXN-01`/`S-TXN-02` — Redesign visual v2.0"); RF-RS-02 AC1-5; RN-18 (preservada) | Composição segue o mapeamento linha a linha; hierarquia RN-18 preservada exatamente (verificação explícita, não presumida); `ShortcutBar`/`ShortcutChip` presente e re-estilizado com tokens v2.0; nenhuma mudança em ranking de atalhos (RN-12), pré-preenchimento (RN-13), filtros disponíveis, ou formulário unificado (RF-REF-04) | 2.0 dias | **Concluída (2026-09-18).** `TransactionsPage.tsx`/`ShortcutChip.tsx`: cabeçalho serifado + subtítulo, `ShortcutBar` no topo, filtros em pílula, card de resumo do período, lista em `Card` único agrupada por dia com ponto de categoria (`--color-chart-N`); RN-18 preservada (linha 1 `neutral-900` semibold; linha 2 `neutral-600`) com teste dedicado. Lacunas de UX-SPEC: chip de período do mockup sem filtro de período no app (não criado); filtros mantidos como `Select` em pílula, não chips clicáveis; paridade mobile só por layout responsivo, sem verificação visual contra `LancamentosMobile.dc.html`. Suíte 451/451, build e oxlint OK. | Lançamentos (Redesign v2.0) |
| FE-RS-08 | Migrar para `<Num />` os pontos de chamada de `formatCentsToBRL()` em `TransactionsPage.tsx` (valor de cada item da lista; card de resumo do período) — `ADR-019`, `DIR-41` | Frontend | `UX-SPEC.md` Seção 2.2 ("Migração do primitivo `Num` neste lote"); `ADR-019` | Todo número de `S-TXN-01` renderiza via `<Num />`; nenhuma regressão de teste existente de Lançamentos | 0.25 dia | **Concluída (2026-09-18).** Valores dos itens e do card de resumo migrados para `<Num format="currency" />`, seta `aria-hidden` como irmão; 0 `formatCentsToBRL` restante no arquivo; 1 matcher de teste ajustado (fragmentação de nós). Suíte 451/451 verificada. | Lançamentos (Redesign v2.0) |
| FE-RS-09 | Redesign visual de `S-ACC-01/02/04` (desktop `ContasCartoes.dc.html` + mobile): migrar de Padrão A (lista) para Padrão C (grade de cards, `AccountCard` novo, mesma família visual de `CategoryCard`/`BudgetCard`) conforme recomendação da `UX-SPEC.md`, confirmada em `AMB-17` — `DET-15` | Frontend | `UX-SPEC.md` Seção 2.2 ("`S-ACC-01/02/04` + `S-CARD-01/02/03` — Redesign visual v2.0"); RF-RS-03 AC1/AC4; `AMB-17` | Grade de `AccountCard` (`iconChip`+rótulo do banco em `--text-2`+nome+saldo) substitui a lista Padrão A; grade colapsa 1→2→3→4 colunas (Padrão C); nenhuma mudança de dado/comportamento de RF-MVP-01/RN-08; ações de editar/inativar/excluir preservadas | 1.5 dias | **Concluída (2026-09-18).** Novo `AccountCard.tsx` (iconChip decorativo, tipo em `neutral-600`, nome, badge Inativa, saldo, botões Editar/Excluir com aria-label); `AccountsPage.tsx` passa a grade `grid-cols-1 sm:2 lg:3 xl:4`. CRUD e fluxo RN-08 inalterados. Executado em worktree isolada (`lote3-contas-cartoes`). | Contas & Cartões (Redesign v2.0) |
| FE-RS-10 | Redesign visual de `S-CARD-01/02` (visão-resumo apenas — grade `grid-cols-2`, nome do cartão + vencimento, fatura atual em destaque, barra de limite usado com percentual+valor separados) — `InvoiceTimeline`/`S-CARD-03` **não é tocado** (permanece com o Lote 8, AMB-17 confirmado). Corrigir, no mesmo arquivo, o achado de consistência já registrado (`UX-SPEC.md` Seção 3.3): trocar o `Input type="number"` do campo "Limite (R$)" pelo `CurrencyInput` já designado — `DET-14` | Frontend | `UX-SPEC.md` Seção 2.2 (idem); RF-RS-03 AC1-4; achado de consistência 2026-09-04 (`UX-SPEC.md` Seção 3.3) | Grade de cartão conforme mapeamento; nenhuma mudança em RN-01 (fechamento)/RN-06 (limite)/horizonte de faturas/`InvoiceTimeline`; campo "Limite (R$)" de `S-CARD-02` usa `CurrencyInput`, não `Input` genérico (teste dedicado); 0 regressões de N3/N4, com atenção redobrada dado o risco de regressão mais alto do Grupo A (RF-RS-03 AC3) | 1.5 dias | **Concluída (2026-09-18).** `CreditCardsPage.tsx`: grade `md:grid-cols-2` (1 coluna no mobile), nome + `vence DD/MM`, fatura atual em destaque, barra de limite usado com percentual e valor em nós separados. Campo Limite já usava `CurrencyInput` (só ganhou teste dedicado). `InvoiceTimeline`/RN-01/RN-06 intocados. Desvio: fatura atual calculada no cliente (1 `listInvoicesByCard` por cartão + `listTransactions`, best-effort) — alternativa é expor o total no endpoint de limite (Backend). | Contas & Cartões (Redesign v2.0) |
| FE-RS-11 | Migrar para `<Num />` os pontos de chamada de `formatCentsToBRL()` em `AccountsPage.tsx` (saldo) e `CreditCardsPage.tsx` (fatura atual; limite usado, separando percentual e valor monetário hoje concatenados) — `ADR-019`, `DIR-41` | Frontend | `UX-SPEC.md` Seção 2.2 ("Migração `Num` neste lote"); `ADR-019` | Todo número de Contas & Cartões renderiza via `<Num />`; percentual e valor do "limite usado" são dois `<Num />` distintos; nenhuma regressão de teste existente | 0.5 dia | **Concluída (2026-09-18).** Saldo em `AccountCard` e fatura atual/limite usado/disponível em `CreditCardsPage` migrados para `<Num />`; percentual e valor do limite são nós distintos; 0 `formatCentsToBRL` restante. 1 matcher de teste ajustado (fragmentação de nós). Suíte 459/459, build e oxlint sem erro. | Contas & Cartões (Redesign v2.0) |
| FE-RS-12 | Aplicar os tokens v2.0 (cor/tipografia/radius/elevação da Seção 3.1) ao `CategoryCard` já existente (Padrão C) — **sem reverter para lista-árvore** (Bloqueio 023 resolvido, `RF-RS-04` AC1 reforçado): dado exibido, clique (expandir/navegar) e ações de edição/exclusão permanecem intocados (RF-RS-04 AC2) | Frontend | `UX-SPEC.md` Seção 2.2 ("`S-CAT-01` — Redesign visual v2.0"); RF-RS-04 AC1-2 | `CategoryCard` renderiza com os tokens v2.0 (cor/borda/elevação/tipografia), mantendo exatamente a mesma estrutura/comportamento já validado desde o Pacote de Refinamento; nenhuma regressão de teste existente de Categorias | 0.5 dia | Concluída — tokens v2.0 (elevação `sm`, `rounded-lg`, `bg-surface`) já herdados via `Card` do Lote 0; nenhuma mudança estrutural/comportamental no `CategoryCard` (status corrigido pelo Validador em 2026-09-18: linha estava desatualizada) | Categorias (Redesign v2.0) |
| FE-RS-13 | Migrar para `<Num />` o valor monetário do `CategoryCard` (total gasto no mês) — `ADR-019`, `DIR-41` | Frontend | `UX-SPEC.md` Seção 2.2 (idem); `ADR-019` | Valor do card renderiza via `<Num format="currency" />`; nenhuma regressão de teste existente | 0.25 dia | Concluída — `CategoryCard` usa `<Num value={totalSpentCents} format="currency" />` (texto " este mês" fora do primitivo); testes de CategoryCard e CategoriesPage ajustados (query por regex, valor agora em span próprio) | Categorias (Redesign v2.0) |
...
### 3.7 Refatoração de Lote (Débitos Técnicos)

**Nova subseção — 2026-09-05.** Mecanismo do Coordenador (chapéu Tech Lead) para
achado simples/não bloqueante reportado pelo Validador sobre um lote já fechado:
vira tarefa formal aqui, num lote nomeado `Refatoração Lote-<origem>`, nunca só uma
nota solta no `QA-REPORT.md`/`SECURITY-REVIEW.md` sem tarefa correspondente. O lote
de origem não é reaberto por causa disso. Convenção de ID desta subseção, ainda sem
precedente no projeto: `<Time>-DEBT-NN` (distinta de `BE/FE/QA-M/F2/F3-NN` e de
`BE/FE/QA-REF-NN`, que já designa o Pacote de Refinamento Fase 2.1 — evita colisão
de significado entre "refinamento de produto" e "débito técnico apontado pelo
Validador").
...
| ID | Tarefa | Time | Origem (componente/tela) | Critério de Aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| FE-DEBT-01 | Cobertura de teste do branch opcional de WebAuthn: adicionar `PinSetupPage.test.tsx` (estados vazio/erro do fluxo de PIN) e teste de `frontend/src/lib/auth/webauthn.ts` com `@simplewebauthn/browser` mockado (mesmo padrão de mock já usado para a API do Supabase em `testSupabaseClient.ts`), cobrindo `registerWebAuthnCredential()`/`authenticateWithWebAuthn()`, hoje exercitados só indiretamente via `AuthGate.test.tsx` | Frontend | `QA-REPORT.md` Seção 15 (`QA-DEBT-015`), tarefa afetada `FE-M-04` | Teste automatizado novo cobre `PinSetupPage.tsx` (não só a transição de estágio via `AuthGate`) nos estados vazio/erro do fluxo de PIN; teste de `webauthn.ts` com `@simplewebauthn/browser` mockado exercita `registerWebAuthnCredential()`/`authenticateWithWebAuthn()` (sucesso e falha, incluindo o fallback automático para PIN); regressão completa da suíte Vitest revalidada sem resíduo | 0.25 dia | **Concluída (2026-09-18).** Testes novos passando; suíte completa sem resíduo. | Refatoração Lote-Autenticação & Segurança |

Achado de origem: severidade Baixa, sem urgência (`QA-REPORT.md` Seção 15.4,
"considerar corrigir na próxima tarefa que tocar `PinSetupPage.tsx`/`webauthn.ts`");
...
#### Refatoração Lote-Dashboard (Redesign v2.0)

| ID | Tarefa | Time | Origem (componente/tela) | Critério de Aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| FE-DEBT-03 | Cobertura de teste permanente do Dashboard v2.0: (1) asserção `axe`/`toHaveNoViolations()` (jest-axe já disponível em `test/setup.ts`) em `DashboardPage.test.tsx` e `DonutChart.test.tsx`; (2) asserção das classes de layout responsivo de `FE-RS-05` (`lg:grid-cols-5` com `lg:col-span-3`/`lg:col-span-2` na Linha 1; `lg:grid-cols-2` na Linha 2; `grid-cols-3` nos KPIs) para travar regressão do wireframe da `UX-SPEC.md` Seção 2.2 | Frontend | `QA-REPORT.md` Seção 27 (achados S-1, S-2), tarefas afetadas `FE-RS-05`, `FE-RS-06`, `FE-RS-15` | Testes novos passam; suíte completa sem resíduo | 0.25 dia (prazo: antes do fechamento do Lote 2) | **Concluída (2026-09-18).** Testes novos passando; suíte completa sem resíduo. | Refatoração Lote-Dashboard (Redesign v2.0) |

#### Refatoração Lote-Contas & Cartões (Redesign v2.0)

| ID | Tarefa | Time | Origem (componente/tela) | Critério de Aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| FE-DEBT-04 | Fatura atual de `CreditCardsPage` sem N+1/`listTransactions()` irrestrito, a11y e cor: (1) substituir o cálculo no cliente (`listInvoicesByCard` por cartão + `listTransactions()` sem filtro, somando `card_invoice_id` no cliente) por leitura agregada/filtrada (RPC/view existente ou filtro por `card_invoice_id`), mantendo a regra de `InvoiceTimeline`; (2) `aria-label` `Editar <nome>` nos botões "Editar" do card de cartão (como em `AccountCard`); (3) asserção `axe`/`toHaveNoViolations()` em `AccountsPage`/`AccountCard`/`CreditCardsPage` (testes); (4) trocar `text-[26px]` por token/escala tipográfica do Lote 0 | Frontend | `QA-REPORT.md` Seção 28 (achados S-1 a S-4), tarefas afetadas `FE-RS-09`, `FE-RS-10`, `FE-RS-11` | Fatura atual idêntica à atual, sem carregar todas as transações do usuário nem N chamadas por cartão; botões Editar distinguíveis por leitor de tela; testes axe passam; suíte completa sem resíduo | 0.5 dia (prazo: antes do fechamento do Lote 4 ou de qualquer lote que amplie o volume lido em `CreditCardsPage`) | Concluída. Nota: listInvoicesByCards (1 consulta .in) + listTransactions({cardInvoiceIds}) substituem N+1/listTransactions() irrestrito; aria-label Editar <nome>; axe em AccountsPage/AccountCard/CreditCardsPage; text-[26px] -> text-2xl. Suíte 463/463. | Refatoração Lote-Contas & Cartões (Redesign v2.0) |

#### Refatoração Lote-4 (Categorias, Redesign v2.0)
...
| ID | Descrição | Responsável | Origem | Critério de aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| FE-DEBT-06 | (1) Convergir estados de carregamento pré-sessão (`OnboardingGate` skeleton; `AuthGate` spinner, hoje `text-neutral-400`) ao padrão `AuthLayout` — `AuthGate` fica em `lib/auth`, exige autorização explícita do usuário (G-21); (2) estabilizar o teste de lockout de `UnlockPage` (falha sob carga, pré-existente em `main`), sem enfraquecer a asserção; (3) `axe` permanente em `LoginPage` e na fase biometria de `PinSetupPage` | Frontend | `QA-REPORT.md` Seção 30 (S-1 a S-3), tarefas `FE-RS-18`, `FE-RS-19`, `FE-RS-20` | Testes passam; suíte completa determinística em 3 execuções seguidas; nenhum arquivo de `lib/auth` alterado sem autorização | 0.25 dia (prazo: antes do fechamento do Lote 6) | Concluída (loading convergido a AuthLayout, incl. AuthGate por autorização G-21; lockout de UnlockPage estabilizado por sincronização com o fim da verificação assíncrona, asserção intacta; axe em LoginPage e biometria do PinSetupPage; suíte 504/504 x3, tsc e build ok) | Refatoração Lote-5 |
| FE-DEBT-07 | Estabilizar testes de UI que estouram o timeout de 5s sob carga da máquina (achado da validação de FE-DEBT-06): CaptureFab (confirmar rascunho), DraftReviewBanner (falha ao confirmar), GoalsPage (aporte), PaymentMethodsPage (cadastro), TransactionsPage (fluxo normal), SettingsPage (alterar PIN), AccountsPage/CreditCardsPage/ExportReportPage e PinSetupPage (divergente, reproduzido também em `main`). Causa provável: `testTimeout` padrão de 5s com `userEvent` + jsdom + axe em `setup.ts`; avaliar `testTimeout`/`hookTimeout` global e limite de workers em `vitest.config`, ou `waitFor`/`findBy` sincronizando o fim de cada fluxo, sem enfraquecer asserções | Frontend | `QA-REPORT.md` Seção 31 (S-1) | Suíte completa sem timeout em 3 execuções seguidas sob carga moderada (com outros processos Node ativos); nenhuma asserção removida ou relaxada | 0.25 dia (prazo: antes do fechamento do Lote 6) | Concluída | Refatoração Lote-5 |

#### Refatoração Lote-6 (Orçamento, Redesign v2.0)

| ID | Descrição | Responsável | Origem | Critério de aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| FE-DEBT-08 | Glifo ⚠ do `ProgressBar` (nível `warning`) com contraste 2,73:1 sobre `warning-soft` (< 3:1, WCAG 1.4.11) e classe arbitrária `text-[color:var(--color-warning)]`: usar tom mais escuro ou utilitário de token que atinja ≥ 3:1 sobre `surface` e `warning-soft`, sem reintroduzir `text-warning` como cor de texto (DIR-44) | Frontend | `QA-REPORT.md` Seção 32 (QA-DEBT-011), `FE-RS-23` | Glifo ≥ 3:1 sobre `surface` e `warning-soft` (cálculo registrado); zero classe arbitrária de cor no componente; `ProgressBar.test.tsx` e `DashboardPage.test.tsx` sem alteração de asserção; axe nos 3 níveis; build sem erro | 0.25 dia (prazo: antes do fechamento do Lote 7) | Não iniciada | Refatoração Lote-6 |
| FE-DEBT-09 | Teste de retorno de foco ao card de origem após fechar o `Modal` (novo/edição) e o `ConfirmationDialog` em `BudgetPage`, sem mudar comportamento; se o retorno de foco não existir hoje, registrar como achado em vez de implementar | Frontend | `QA-REPORT.md` Seção 32 (QA-DEBT-012), `FE-RS-28` | Teste novo cobre o retorno de foco (ou achado documentado); nenhuma asserção existente alterada; suíte de `src/pages/budget` verde | 0.25 dia (prazo: antes do fechamento do Lote 7) | Não iniciada | Refatoração Lote-6 |
...
| Tarefa | Depende de | Tipo | Pode rodar em paralelo com |
|---|---|---|---|
| FE-DEBT-01 | Nenhuma (lote de origem, "Autenticação & Segurança", já `Concluída`/fechado, Seção 7) | Implementação completa | Qualquer tarefa de qualquer outro lote — sem sobreposição de arquivo/domínio esperada |

**Posição na ordem de execução geral**: este lote roda **depois de todos os lotes
já existentes** nesta Seção 4 (4.1 a 4.5), inclusive os ainda pendentes do Grupo A
...
#### Refatoração Lote-Captura Automatizada — Voz & Foto

**Origem**: `SECURITY-REVIEW.md` Seção 1.31 (auditoria completa de lote,
2026-09-08), achado `SEC-DEBT-015` — não bloqueante, mas com condição de
bloqueio automático futuro (mesmo padrão já usado por `SEC-DEBT-002`/`006`).

| ID | Tarefa | Time | Origem (componente/tela) | Critério de Aceite | Estimativa | Status | Lote |
|---|---|---|---|---|---|---|---|
| BE-DEBT-04 | Corrigir `candidate_transaction.import_batch_id` para `ON DELETE SET NULL` (hoje `ON DELETE CASCADE`, migration `20260904170000_be_f3_00_...sql`) — alinhar ao comportamento já documentado em `API-CONTRACT.yaml` (`DELETE /import_batch?id=eq.{id}`: "nenhum candidato é apagado em cascata pelo lote"). Migration aditiva nova (não editar a já aplicada): `ALTER TABLE candidate_transaction DROP CONSTRAINT ..., ADD CONSTRAINT ... FOREIGN KEY (import_batch_id) REFERENCES import_batch(id) ON DELETE SET NULL` | Backend | `SECURITY-REVIEW.md` Seção 1.31 (`SEC-DEBT-015`), RNF-08, `API-CONTRACT.yaml` `/import_batch?id=eq.{id}` | Excluir um `import_batch` nunca apaga nenhuma linha de `candidate_transaction` vinculada (mesmo uma já `confirmed`/`discarded`) — `import_batch_id` vira `NULL`, preservando `raw_payload`/`confirmed_at`/`resulting_transaction_id` do candidato; teste SQL novo reproduz o cenário (criar lote → candidato → confirmar via RPC → excluir o lote → candidato ainda existe com `import_batch_id IS NULL`) | 0.5 dia | Concluída (migration `20260918100000_be_debt_04_candidate_import_batch_set_null.sql` + teste `supabase/tests/be_debt_04_candidate_import_batch_set_null.test.sql`; coluna já era nullable; teste não executado localmente, sem supabase CLI/DB) | Refatoração Lote-Captura Automatizada — Voz & Foto |

**Posição na ordem de execução geral**: este lote roda **depois de todos os
lotes já existentes** nesta Seção 4, incluindo os 5 lotes de Refatoração já
listados acima — mesmo racional: débito técnico não-crítico sobre um lote já
fechado (voz/foto nunca populam `import_batch_id`, então o gap é hoje
inexploitável pelo fluxo real deste lote; só passa a importar de fato quando
`BE-F3-03`/`BE-F3-04` — Importação de Extrato/Open Finance — começarem a gerar
candidatos vinculados a lotes reais). **Condição de bloqueio automático**: esta
tarefa deve estar `Concluída` **antes** de `BE-F3-03`/`BE-F3-04` (Seção 3.3)
serem consideradas prontas para produção — mesmo mecanismo de gate já usado
para `SEC-DEBT-002`/`006` (Seção 3.3, nota de pré-condição do bloco).
...
— ver racional em 7.9 abaixo. **Décimo lote fechado, mesma data**
("Autenticação & Segurança", MVP) — primeira aplicação formal do "Critério de
...
— ver racional em 7.11 abaixo. **Lote "Captura Automatizada — Voz & Foto"
fechado em 2026-09-08** (nota de validação em Seção 3, linha acima da tabela
de tarefas do lote) e **publicado em staging em 2026-09-09** (`/deploy`,
`DEPLOY.md` §9.13) — ver racional de fechamento/deploy em 7.16 abaixo.
**Lote "Retenção & Descarte de Dado / Exclusão de Conta" fechado em
...
| Captura Automatizada — Importação de Extrato | BE-F3-03, FE-F3-05 | 2026-09-15 | Aprovado (2/2, nenhuma reprovação — `QA-REPORT.md` Seção 26.5) | Aprovado com débito (`SEC-DEBT-015`, Média, condição de bloqueio de **produção** confirmada ativa nesta rodada, `BE-DEBT-04` segue `Não iniciada` — `SECURITY-REVIEW.md` Seção 1.36) | `BE-DEBT-04` (lote "Refatoração Lote-Captura Automatizada — Voz & Foto", Seção 3.7, `Não iniciada`) — mesmo débito já registrado em 1.31 para o lote "Captura Automatizada — Voz & Foto", agora com a condição de bloqueio de produção efetivamente atingida, não um débito novo | **Não publicado.** Nenhum deploy realizado para este lote nesta rodada — `/deploy` em **staging** liberado (achado é Média, não bloqueia staging); `/deploy` em **produção** bloqueado até `BE-DEBT-04` `Concluída` (`SECURITY-REVIEW.md` Seção 1.36, requisito de segurança operacional para o DevOps). **Atualização 2026-09-18**: frontend publicado em STAGING (`dpl_8TPzeWYtYv59tQHTBo6n6em2t1mk`, `DEPLOY.md` §9.14); function `statement-import` NÃO publicada no Supabase (pendente, confirmação humana). Produção segue bloqueada |
| Relatórios & Exportação (Fase 3) | ver Seção 3 (nota de validação, 2026-09-15) | 2026-09-15 | ver `QA-REPORT.md` | ver `SECURITY-REVIEW.md` | ver Seção 3 | **STAGING (frontend) publicado 2026-09-18** — `dpl_8TPzeWYtYv59tQHTBo6n6em2t1mk`, `DEPLOY.md` §9.14. Function `report-export` NÃO publicada no Supabase (pendente, confirmação humana). Produção: não realizado |
| Dashboard (Redesign v2.0), Lote 1 | FE-RS-05, FE-RS-06, QA-RS-04 | ver Seção 3 (commit 091e697) | ver `QA-REPORT.md` | ver `SECURITY-REVIEW.md` | ver Seção 3 | **STAGING (frontend) publicado 2026-09-18** — `dpl_8TPzeWYtYv59tQHTBo6n6em2t1mk`, `DEPLOY.md` §9.14. Sem dependência de backend novo. Produção: não realizado |
...
### 7.10 Racional de fechamento — lote "Autenticação & Segurança" (MVP)

Apliquei o "Critério de Aprovação de Lote" item a item, mesmo rigor de 7.1-7.9.

1. **Toda tarefa `Concluída`**: confirmado por leitura direta da Seção 3.1 — as 9
   tarefas (`BE-M-09`, `11`, `12`, `13`, `14`, `FE-M-04`, `12`, `13`, `QA-M-02`)
   têm status `Concluída`, todas já pertencentes ao lote "Autenticação &
   Segurança". Nenhuma dependência da Seção 4.1 ficou órfã (todas as tarefas
   citadas como pré-requisito de cada uma — `BE-M-00` a `BE-M-12` — também estão
   `Concluída`); nenhuma tarefa `Bloqueada`. Passa.
2. **`QA-REPORT.md` Aprovado/Aprovado com ressalvas**: Seção 15.7, texto literal
   "Veredito de lote... Aprovado" — 9/9, nenhuma reprovação. O único achado novo
   (`QA-DEBT-015`, cobertura de teste do branch opcional de WebAuthn) é
   classificado pelo próprio QA como não-bloqueante ("não reduz o veredito").
   Passa, sem ressalva de lote.
3. **`SECURITY-REVIEW.md` Aprovado/Aprovado com débito**: Seção 1.25, "Veredito
   final do lote... Aprovado, com débito de baixa severidade (`SEC-DEBT-001`),
   não bloqueante." `SEC-DEBT-001` é achado pré-existente (CORS wildcard em
   `auth-email-mfa`, já rastreado desde `BLOCKERS.md` Bloqueio 009), só
   reclassificado de Média para Baixa nesta rodada — não gera tarefa nova (só o
   achado novo, `QA-DEBT-015`, gera). Passa.
4. **Nenhum `BLOCKERS.md` `Aberto` afetando o lote**: nenhum bloqueio hoje aberto
   cita as 9 tarefas deste lote por nome. Passa.
5. **Nenhuma diretriz da Seção 1 violada sem exceção registrada**: sem achado novo
   nesta rodada que aponte violação de diretriz vigente. Passa.
6. **Esforço real reconciliado com a estimativa original**: as 9 tarefas somam a
   estimativa já registrada na Seção 3.1/Seção 5 ao longo das rodadas de
   2026-09-02/03/04; nenhum desvio grande de escopo/estimativa reportado nesta
   rodada de validação (só confirmação de trabalho já concluído). Passa.

**Achado tratado — não decidido em silêncio**: `QA-DEBT-015` virou tarefa formal
`FE-DEBT-01`, lote novo `Refatoração Lote-Autenticação & Segurança` (Seção 3.7,
Seção 4.6), posicionado depois de todos os lotes já existentes na ordem de
execução — o lote de origem ("Autenticação & Segurança") não é reaberto por
causa dele, conforme o mecanismo de achado simples/não bloqueante deste agente.

**Deploy**: mesmo padrão já registrado em 7.5 para o lote "Orçamento" — este
lote já está em produção desde 2026-09-03, como parte da mesma promoção mais
ampla autorizada pelo stakeholder (`DEPLOY.md` §9.6/§9.7), anterior à validação
formal QA/DevSecOps por lote existir para "Autenticação & Segurança"
especificamente. Este registro de Seção 7 formaliza retroativamente a dupla
aprovação — não dispara um novo deploy.
...
### 7.16 Racional de fechamento e deploy — lote "Captura Automatizada — Voz & Foto" — 2026-09-08/09

Fechamento formal (checagem estrutural, feita pelo próprio Validador,
`/validar`, 2026-09-08 — registrado inline na nota de validação acima da
tabela de tarefas do lote, Seção 3): 7/7 tarefas `Concluída`, dependência da
Seção 4.3 (`FE-F3-04 | FE-F3-02, FE-F3-03, BE-F3-00`) confirmada não-órfã,
nenhuma tarefa `Bloqueada`. `QA-REPORT.md` Aprovado (7/7, Seção 21.6);
`SECURITY-REVIEW.md` Aprovado com débito (`SEC-DEBT-015`, Média, Seção
1.31) — débito já agendado como `BE-DEBT-04` no lote de refatoração
correspondente (Seção 3.7), não deixado como nota solta. Lote fecha
**Validado (com ressalvas)**.

**Deploy (2026-09-09, `/deploy`, `DEPLOY.md` §9.13)**: validação final de
confirmação (releitura do `git diff` desde o veredito acima) não encontrou
nenhuma mudança tocando as 7 tarefas aprovadas — toda alteração não
commitada na árvore de trabalho nesta data pertence a lotes-irmãos
diferentes (`BE-F3-03` a `BE-F3-08`) e é estritamente aditiva onde toca os
mesmos arquivos. Frontend publicado em staging no destino novo
`objetivo-financeiro-ljs-staging.vercel.app` (migração de hospedagem
`mymoney` → `objetivo-financeiro-ljs`, decisão do stakeholder — ver
`BLOCKERS.md` Bloqueio 004). Dois achados novos, registrados com
transparência, nenhum deles reabre o veredito de QA/DevSecOps acima (nenhum
é regressão de código, ambos são lacunas de execução/infraestrutura
descobertas só agora): (1) incidente de primeiro-deploy-em-produção do
projeto novo, contido, sem ação adicional de produção; (2) Edge Functions
`receipt-ocr`/`voice-capture` (`BE-F3-01`/`BE-F3-02`) nunca publicadas no
Supabase real — `BLOCKERS.md` Bloqueio 025, dono Backend, não-bloqueante
para este registro (degrada a preenchimento manual, caminho já auditado),
mas pendente antes da funcionalidade de voz/foto estar de fato operante.

**Deploy em produção real**: não realizado — pausa obrigatória do
`EXECUTION-FLOW.md` Comando 3, Seção 5, permanece em vigor, sem confirmação
explícita do orquestrador/usuário ainda coletada.

### 7.17 Racional de fechamento — lote "Retenção & Descarte de Dado / Exclusão de Conta" — 2026-09-15

Fechamento formal (checagem estrutural, feita pelo próprio Validador,
`/validar`, 2026-09-15 — registrado inline na nota de validação acima da
tabela "### 3.4", Seção 3.3): 5/5 tarefas `Concluída`; dependências da
Seção 4.3 conferidas — `BE-F3-09` (linha "BE-F3-00/04 (contrato)") tem uma
lacuna documentada (Open Finance/`BE-F3-04` ainda `Não iniciada`), mas é a
mesma classe de lacuna futura já aceita para `BE-F3-08`/foto de recibo
(`ADR-020`), explicitamente descrita no cabeçalho da própria migration —
**não é dependência órfã/inconsistente**, é sequenciamento correto de uma
tabela que ainda não existe; `FE-F3-09` (pendência externa de UX-SPEC.md,
item `UX-01`, Seção 6.1.1) segue não-bloqueante, como já registrado desde a
resolução original do `ADR-011`. Nenhuma tarefa `Bloqueada`. `QA-REPORT.md`
Aprovado (5/5, Seção 22.5); `SECURITY-REVIEW.md` Aprovado, sem débito de
segurança novo (Seção 1.32) — leitura linha a linha da superfície de maior
consequência do lote (`delete_user_data`/`delete-account`) não encontrou
achado de nenhuma severidade.

**Pendência operacional (não achado de código, não gera `Refatoração
Lote-X`)**: as 2 migrations deste lote não foram aplicadas ao projeto
Supabase real vinculado — mesma natureza do `BLOCKERS.md` Bloqueio 025
(bloqueio do classificador de permissão do ambiente do Executor, não decisão
de arquitetura). Requisito operacional detalhado registrado em
`SECURITY-REVIEW.md` Seção 4, endereçado ao chapéu DevOps no próximo
`/deploy` (Comando 3, Seção 1/2) — aplicar as migrations, publicar
`delete-account`, reexecutar `be_f3_08_data_retention_purge_jobs.test.sql`/
`be_f3_09_delete_account_data.test.sql` contra o projeto real, antes de
qualquer smoke test funcional de exclusão de conta em staging.
