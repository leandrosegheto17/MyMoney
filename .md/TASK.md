...
|---|---|---|---|---|---|---|---|
| FE-DEBT-04 | Cobertura de teste permanente de acessibilidade do `CategoryCard`: asserção `axe`/`toHaveNoViolations()` em `CategoryCard.test.tsx` (sonda ad hoc do Validador passou sem violações) | Frontend | `QA-REPORT.md` Seção 28 (achado S-1), tarefas `FE-RS-12`, `FE-RS-13` | Teste novo passa; suíte completa sem resíduo | 0.1 dia (prazo: antes do fechamento do Lote 5) | Não iniciada | Refatoração Lote-4 |
